...
import {
  estimateRouteDistanceNm,
  type RouteDistanceSource,
} from "./routeDistances";
...
export interface AirlineBankProfile {
  ...
  /** Carrier-specific markets prevent implausible airline/route pairings. */
  markets?: Partial<Record<OperationTrafficClass, string[]>>;
  bankMultipliers: Record<
    string,
    Partial<Record<"arrival" | "departure", number>>
  >;
...
const INTERNATIONAL_FLEET: WeightedAircraft[] = [
  { model: "B789", weight: 0.27 },
  { model: "A359", weight: 0.24 },
  { model: "A333", weight: 0.22 },
  { model: "B77W", weight: 0.21 },
...
const HUB_BANKS = {
  ...
  string,
  Record<string, Partial<Record<"arrival" | "departure", number>>>
>;
...
function source(
  title: string,
  url?: string,
  role: AirportTrafficProgramSource["role"] = "airline-directory",
): AirportTrafficProgramSource {
  return { title, url, retrievedOn: "2026-07-25", role };
}
  ...
  return {
    ...
    airlines: [
      ...airlines,
      {
        ...
        bankMultipliers: {},
        gate: {
          zoneNames: ["General Aviation", "Remote Ramp"],
          label: "general-aviation / remote stands",
        },
      },
    ],
    markets: {
    ...
    sources,
    fidelity: sources.some((item) => item.role !== "schematic-design")
      ? "sourced-airlines-schematic-weights"
      : "schematic",
  };
}
  ...
  ORD: program(
    ...
    [
      passengerAirline(
        ...
        0.47,
        {
          concourses: ["B", "C", "E", "F", "G"],
          label: "United B/C/E/F/G allocation",
        },
        DOMESTIC_FLEET,
        true,
        1,
      ...
      passengerAirline(
        ...
        {
          concourses: ["G", "H", "K", "L"],
          label: "American G/H/K/L allocation",
        },
        DOMESTIC_FLEET,
        true,
        0.8,
      ...
      passengerAirline(
        "WN",
        0.045,
        { concourses: ["M"], label: "Terminal 5 common-use allocation" },
        [{ model: "B738", weight: 1 }],
        false,
        0,
        {
          passenger: ["ATL", "BOS", "DEN", "DFW", "MCO"],
        },
      ),
      passengerAirline(
        "DL",
        ...
        0,
        {
          passenger: ["ATL", "BOS", "JFK", "LAX", "SEA"],
        },
      ),
      passengerAirline(
        "BA",
        0.018,
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "B789", weight: 0.45 },
          { model: "A359", weight: 0.25 },
          { model: "B77W", weight: 0.3 },
        ],
        false,
        0,
        { passenger: ["LHR"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "B789", weight: 0.55 },
          { model: "B77W", weight: 0.45 },
        ],
        false,
        0,
        { passenger: ["HND", "NRT"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "B77W", weight: 0.65 },
          { model: "B789", weight: 0.35 },
        ],
        false,
        0,
        { passenger: ["HND", "NRT"] },
      ...
      passengerAirline(
        ...
        [
          { model: "B789", weight: 0.45 },
          { model: "A359", weight: 0.3 },
          { model: "B77W", weight: 0.25 },
        ],
        false,
        0,
        { passenger: ["IST"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "A333", weight: 0.35 },
          { model: "A359", weight: 0.25 },
          { model: "B789", weight: 0.25 },
          { model: "B748", weight: 0.15 },
        ],
        false,
        0,
        { passenger: ["FRA", "MUC"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "B789", weight: 0.65 },
          { model: "B77W", weight: 0.35 },
        ],
        false,
        0,
        { passenger: ["VIE"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "B77W", weight: 0.9 },
          { model: "A359", weight: 0.1 },
        ],
        false,
        0,
        { passenger: ["DXB"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "A359", weight: 0.45 },
          { model: "B77W", weight: 0.55 },
        ],
        false,
        0,
        { passenger: ["CDG"] },
      ...
      passengerAirline(
        "AC",
        0.018,
        { concourses: ["M"], label: "Terminal 5 transborder allocation" },
        [
          { model: "A320", weight: 0.28 },
          { model: "A21N", weight: 0.28 },
          { model: "B789", weight: 0.24 },
          { model: "B77W", weight: 0.2 },
        ],
        false,
        0,
        { passenger: ["YYZ", "YUL", "YVR", "YTZ"] },
      ),
      passengerAirline(
        "EI",
        0.007,
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "A333", weight: 0.7 },
          { model: "A21N", weight: 0.3 },
        ],
        false,
        0,
        { passenger: ["DUB"] },
      ...
      passengerAirline(
        ...
        { concourses: ["M"], label: "Terminal 5 international allocation" },
        [
          { model: "A333", weight: 0.6 },
          { model: "B77W", weight: 0.4 },
        ],
        false,
        0,
        { passenger: ["ZRH"] },
      ),
      cargoAirline("5X", 0.028, {
        zoneNames: ["Southeast Cargo Ramp"],
        label: "Southeast Cargo Ramp",
      }),
      cargoAirline("FX", 0.022, {
        zoneNames: ["Southwest Cargo Ramp", "Southeast Cargo Ramp"],
        label: "south cargo ramps",
      }),
    ],
    {
      passenger: [
    ...
    [
      source(
        "Chicago Department of Aviation O’Hare facility and gate data",
        "https://www.flychicago.com/business/CDA/factsfigures/pages/facility.aspx",
        "terminal-allocation",
      ),
      source(
        "Chicago Department of Aviation O’Hare nonstop international destinations, July 2026",
        "https://www.flychicago.com/SiteCollectionDocuments/O%27Hare/ArchivedPDFs/MyFlight/INTLnonstops.pdf",
      ),
    ],
  ),
  ...
  DFW: program(
    ...
    [
      passengerAirline(
        "AA",
        0.73,
        { standSector: [0, 0.72], label: "American terminal bank" },
        DOMESTIC_FLEET,
        true,
        1,
      ),
      passengerAirline(
        "DL",
        0.045,
        { standSector: [0.72, 0.8], label: "Terminal E bank" },
        DOMESTIC_FLEET,
      ),
      passengerAirline(
        "UA",
        0.04,
        { standSector: [0.8, 0.87], label: "Terminal E bank" },
        DOMESTIC_FLEET,
      ),
      passengerAirline(
        "F9",
        0.025,
        { standSector: [0.87, 0.92], label: "Terminal E bank" },
        [{ model: "A320", weight: 1 }],
      ),
      passengerAirline(
        "BA",
        0.025,
        { standSector: [0.92, 1], label: "Terminal D international bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.07, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.065, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "DFW official airline and gate directory",
  ...
  DEN: program(
    ...
    [
      passengerAirline(
        "UA",
        0.46,
        { standSector: [0, 0.5], label: "United A/B bank" },
        DOMESTIC_FLEET,
        true,
        1,
      ),
      passengerAirline(
        "WN",
        0.31,
        { standSector: [0.5, 0.79], label: "Southwest C bank" },
        [{ model: "B738", weight: 1 }],
        true,
        0,
      ),
      passengerAirline(
        "F9",
        0.13,
        { standSector: [0.79, 0.92], label: "Frontier A bank" },
        [{ model: "A320", weight: 1 }],
        true,
        0,
      ),
      passengerAirline(
        "AA",
        0.035,
        { standSector: [0.92, 0.96], label: "common-use bank" },
        DOMESTIC_FLEET,
      ),
      cargoAirline("FX", 0.035, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.03, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "Denver International official airline directory",
  ...
  DXB: program(
    ...
    [
      passengerAirline(
        "EK",
        0.67,
        { standSector: [0, 0.68], label: "Emirates Terminal 3 bank" },
        INTERNATIONAL_FLEET,
        true,
        0,
      ),
      passengerAirline(
        "FZ",
        0.14,
        { standSector: [0.68, 0.82], label: "flydubai Terminal 2/3 bank" },
        [{ model: "B738", weight: 1 }],
        true,
        0,
      ),
      passengerAirline(
        "BA",
        0.045,
        { standSector: [0.82, 0.88], label: "Terminal 1 international bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "TK",
        0.04,
        { standSector: [0.88, 0.94], label: "Terminal 1 international bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "UA",
        0.02,
        { standSector: [0.94, 1], label: "Terminal 3 partner bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.045, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.04, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "Dubai Airports official airline directory",
        "https://dubaiairports.ae/airlines",
        "terminal-allocation",
      ),
    ],
  ),
  ...
  HND: program(
    ...
    [
      passengerAirline(
        "NH",
        0.43,
        { standSector: [0, 0.45], label: "ANA Terminal 2 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "JL",
        0.41,
        { standSector: [0.45, 0.84], label: "JAL Terminal 1 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "UA",
        0.035,
        { standSector: [0.84, 0.89], label: "Terminal 3 international bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "AA",
        0.03,
        { standSector: [0.89, 0.94], label: "Terminal 3 international bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "BA",
        0.025,
        { standSector: [0.94, 1], label: "Terminal 3 international bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.03, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.03, {
    ...
    [
      source(
        "Haneda Airport official airline list",
        "https://tokyo-haneda.com/en/flight/company_list.html?tab=intFlight",
        "terminal-allocation",
      ),
    ],
  ),
  ...
  LHR: program(
    ...
    [
      passengerAirline(
        "BA",
        0.57,
        { standSector: [0, 0.6], label: "British Airways Terminal 5/3 bank" },
        INTERNATIONAL_FLEET,
        true,
        0.25,
      ),
      passengerAirline(
        "VS",
        0.12,
        { standSector: [0.6, 0.71], label: "Virgin Atlantic Terminal 3 bank" },
        INTERNATIONAL_FLEET,
        true,
        0,
      ),
      passengerAirline(
        "AA",
        0.07,
        { standSector: [0.71, 0.79], label: "oneworld Terminal 3/5 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "UA",
        0.055,
        { standSector: [0.79, 0.86], label: "Terminal 2 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "EK",
        0.045,
        { standSector: [0.86, 0.92], label: "Terminal 3 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "TK",
        0.04,
        { standSector: [0.92, 1], label: "international common-use bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.05, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.05, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "Heathrow official airline and terminal information",
        "https://www.heathrow.com/airline-contact-info/british-airways",
        "terminal-allocation",
      ),
    ],
  ),
  ...
  IST: program(
    ...
    [
      passengerAirline(
        "TK",
        0.74,
        { standSector: [0, 0.76], label: "Turkish Airlines hub bank" },
        INTERNATIONAL_FLEET,
        true,
        0.45,
      ),
      passengerAirline(
        "BA",
        0.04,
        { standSector: [0.76, 0.83], label: "international common-use bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "EK",
        0.04,
        { standSector: [0.83, 0.9], label: "international common-use bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "UA",
        0.025,
        { standSector: [0.9, 0.95], label: "international common-use bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.08, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.075, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "Airport Auto hub-program design informed by Turkish Airlines’ official destination network",
        "https://www.turkishairlines.com/en-int/flight-destinations/",
        "schematic-design",
      ),
    ],
  ),
  ...
  LAX: program(
    ...
    [
      passengerAirline(
        "AA",
        0.19,
        { standSector: [0, 0.18], label: "American Terminal 4 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "DL",
        0.19,
        { standSector: [0.18, 0.37], label: "Delta Terminal 2/3 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "UA",
        0.18,
        { standSector: [0.37, 0.55], label: "United Terminal 7/8 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "WN",
        0.12,
        { standSector: [0.55, 0.66], label: "Southwest Terminal 1 bank" },
        [{ model: "B738", weight: 1 }],
      ),
      passengerAirline(
        "AS",
        0.11,
        { standSector: [0.66, 0.77], label: "Alaska Terminal 6 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "BA",
        0.04,
        { standSector: [0.77, 0.84], label: "international terminal bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "NH",
        0.035,
        { standSector: [0.84, 0.9], label: "international terminal bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "EK",
        0.025,
        { standSector: [0.9, 0.95], label: "international terminal bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("5X", 0.055, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("FX", 0.055, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
    ],
    ...
    [
      source(
        "Los Angeles World Airports official airline locations",
  ...
  JFK: program(
    ...
    [
      passengerAirline(
        "B6",
        0.3,
        { standSector: [0, 0.3], label: "JetBlue Terminal 5 bank" },
        DOMESTIC_FLEET,
        true,
        0.45,
      ),
      passengerAirline(
        "DL",
        0.27,
        { standSector: [0.3, 0.56], label: "Delta Terminal 4 bank" },
        DOMESTIC_FLEET,
        true,
        0.55,
      ),
      passengerAirline(
        "AA",
        0.14,
        { standSector: [0.56, 0.69], label: "American Terminal 8 bank" },
        INTERNATIONAL_FLEET,
        true,
        0.35,
      ),
      passengerAirline(
        "BA",
        0.07,
        { standSector: [0.69, 0.77], label: "oneworld Terminal 8 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "TK",
        0.04,
        { standSector: [0.77, 0.84], label: "Terminal 1 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "EK",
        0.04,
        { standSector: [0.84, 0.91], label: "Terminal 4 bank" },
        INTERNATIONAL_FLEET,
      ),
      passengerAirline(
        "NH",
        0.025,
        { standSector: [0.91, 0.96], label: "Terminal 7 bank" },
        INTERNATIONAL_FLEET,
      ),
      cargoAirline("FX", 0.06, {
        zoneNames: ["Cargo Ramp"],
        label: "cargo ramp",
      }),
      cargoAirline("5X", 0.055, {
    ...
    [
      source(
        "Port Authority JFK official terminal airline directory",
...
const LOCAL_PROGRAM = program(
  ...
  [
    ...
    passengerAirline(
      "WN",
      0.12,
      { standSector: [0, 1], label: "visiting airline stand" },
      [{ model: "B738", weight: 1 }],
      false,
      0,
    ),
    ...
    cargoAirline("FX", 0.1, {
      zoneNames: ["Cargo Ramp", "Remote Ramp"],
      label: "local cargo / remote stand",
    }),
    cargoAirline("5X", 0.08, {
      zoneNames: ["Cargo Ramp", "Remote Ramp"],
      label: "local cargo / remote stand",
    }),
  ],
  ...
  [
    source(
      "Airport Auto procedural local-field program",
      undefined,
      "schematic-design",
    ),
  ],
...
export function airportTrafficProgram(
  airportCode: string,
): AirportTrafficProgram {
...
export function selectTrafficProgram(
  ...
  const candidates = program.airlines
    .map((airline) => {
      ...
      const bankMultiplier =
        airline.bankMultipliers[input.periodId]?.[input.direction] ?? 1;
      return {
        airline,
        bankMultiplier,
        weight: airline.baseWeight * classWeight * bankMultiplier,
      };
    })
    .filter((candidate) => candidate.weight > 0);
  const selected =
    weightedChoice(candidates, input.flightId, input.airportSeed, 0x51f15e) ??
    candidates[0];
  const fallbackAirline =
    program.airlines.find((airline) => airline.airline === "LOCAL") ??
    program.airlines[0];
  ...
  const routeOptions = markets.map((market, marketIndex) => ({
    market,
    route: estimateRouteDistanceNm(
      program.airportCode,
      market,
      input.trafficClass,
      input.flightId + marketIndex * 101,
      input.airportSeed,
    ),
  }));
  ...
  const routePool = viableRoutes.length ? viableRoutes : routeOptions;
  const routeChoice =
    routePool[
      Math.floor(
        deterministicUnit(input.flightId, input.airportSeed, 0xc0ffee) *
          routePool.length,
      ) % routePool.length
    ];
  const market = routeChoice.market;
  const route = routeChoice.route;
  const requiredRangeNm = route.distanceNm * 1.08 + 180;
  const routeCapableFleet = fleet.filter(
    (candidate) =>
      aircraftProfile(candidate.model).maximumRangeNm >= requiredRangeNm,
  );
  const compatibleFleet = routeCapableFleet.filter(
    (candidate) =>
      !input.supportsAircraft || input.supportsAircraft(candidate.model),
  );
  const routeCapableDefault = defaultFleet(input.trafficClass).filter(
    (candidate) =>
      aircraftProfile(candidate.model).maximumRangeNm >= requiredRangeNm &&
      (!input.supportsAircraft || input.supportsAircraft(candidate.model)),
  );
  ...
  const aircraft =
    weightedChoice(aircraftPool, input.flightId, input.airportSeed, 0xa17c9e)
      ?.model ??
    (input.trafficClass === "general-aviation"
      ? "PC12"
      : input.trafficClass === "regional"
        ? "E175"
        : input.trafficClass === "cargo"
          ? "B77F"
          : "A320");
  return {
...
export function airlineGatePreference(
  ...
  return (
    airportTrafficProgram(airportCode).airlines.find(
      (airline) =>
        airline.airline === airlineCode &&
        (trafficClass === undefined ||
          (airline.classWeights[trafficClass] ?? 0) > 0),
    )?.gate ?? null
  );
}
...
export function airlineBankWeight(
  ...
  const airline = airportTrafficProgram(airportCode).airlines.find(
    (candidate) => candidate.airline === airlineCode,
  );
  if (!airline) return 0;
  return (
    airline.baseWeight *
...
function weightedChoice<T extends { weight: number }>(
  ...
  const total = values.reduce(
    (sum, value) => sum + Math.max(0, value.weight),
    0,
  );
