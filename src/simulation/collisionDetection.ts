...
const obstacleBoundsCaches = new WeakMap<
...
const obstacleSpatialIndexes = new WeakMap<
  AirportConfig,
...
const collisionEnvelopeSampleCaches = new WeakMap<
  Flight,
  CollisionEnvelopeSampleCache
>();
...
export function parkedAircraftBodyRadius(
  scope: AirportConfig["scope"],
  model: AircraftModel,
): number {
...
export function aircraftCollisionEnvelope(
  config: AirportConfig,
  flight: Flight,
  progress = flight.progress,
): AircraftCollisionEnvelope {
  if (collisionPerformanceTrace)
    collisionPerformanceTrace.envelopeRequests += 1;
  const p = clamp(progress, 0, 1);
  if (collisionSamplingActive) {
    const cached = collisionEnvelopeSampleCaches.get(flight);
    const result =
      cached?.epoch === collisionSamplingEpoch
        ? cached.samples.get(p)
        : undefined;
    if (result) {
      if (collisionPerformanceTrace)
        collisionPerformanceTrace.envelopeCacheHits += 1;
      return result;
    }
  }
  if (collisionPerformanceTrace)
    collisionPerformanceTrace.envelopeComputations += 1;
  ...
  const presentationScale =
    flight.phase === "approach" ? approachScale : takeoffScale;
  const halfLength =
    ((aircraft.visual.bodyLength + aircraft.visual.bodyRadius * 2) / 2) *
    presentationScale;
  ...
  const bodyRadius = Math.max(
    aircraft.visual.bodyRadius * presentationScale,
    halfLength,
    halfWidth,
  );
  const envelope = (
    ...
    minimumAltitude:
      values.altitude -
      Math.max(
        config.scope === "center" ? 0.35 : 1.4,
        aircraft.visual.bodyRadius * 1.8 * presentationScale,
      ),
    maximumAltitude:
      values.altitude +
      Math.max(
        config.scope === "center" ? 0.32 : 1.2,
        aircraft.visual.tailHeight * presentationScale,
      ),
    ...values,
  });
  const sampled = (
    result: AircraftCollisionEnvelope,
  ): AircraftCollisionEnvelope => {
    if (!collisionSamplingActive) return result;
    const cached = collisionEnvelopeSampleCaches.get(flight);
    if (cached?.epoch === collisionSamplingEpoch) cached.samples.set(p, result);
    else
      collisionEnvelopeSampleCaches.set(flight, {
  ...
  if (phaseUsesFlightTrajectory(flight.phase)) {
    ...
    const motion =
      Math.abs(p - flight.progress) < 1e-9 && flight.motion
        ? flight.motion
  ...
  const standNode = stand
    ? surfaceNodeIndex(config.surfaceGraph).get(stand.nodeId)
    : undefined;
  const gate = standNode
    ? { x: standNode.position[0], y: standNode.position[1] }
    : gatePoint(config, flight.gateSlot);
  ...
  const routeSample = sampleSurfaceRouteWithEdges(
    config.surfaceGraph,
    flight.surfaceRoute,
  ...
  if (routeSample) {
    ...
    const protectedSurface =
      routeSample.edge?.kind === "runway" ||
      routeSample.edge?.kind === "runway-access";
    return sampled(
      envelope({
        x: motion.x,
        y: motion.y,
...
export function detectFlightConflict(
  ...
  if (horizontalDistance < physicalRequired && physicalVerticalOverlap) {
    const sameProtectedRunway =
      first.runway === second.runway &&
      first.protectedSurface &&
      second.protectedSurface;
    const type =
      first.airborne && second.airborne
        ? "airborne"
  ...
  if (first.airborne && second.airborne) {
    ...
    if (
      horizontalDistance < requiredHorizontal &&
      verticalDistance < AIRBORNE_VERTICAL
    ) {
      return {
        type: "airborne",
        first: first.id,
  ...
  if (first.airborne !== second.airborne) {
    ...
    // `first` or `second` is the surface proxy, whose altitude is always
    // close to the pavement. Looking at the minimum of both altitudes would
    // therefore report an incursion for an aircraft that is still safely high
    // above a crossing taxiway. The airborne envelope is the meaningful
    // vertical gate; use its lower bound so flare/rollout protection begins
    // before the centre point reaches the threshold altitude.
    const airborne = first.airborne ? first : second;
    if (
      horizontalDistance < AIR_SURFACE_HORIZONTAL &&
      airborne.minimumAltitude < AIR_SURFACE_ALTITUDE
    ) {
      return {
  ...
  const sameTaxiway =
    bothActiveMovers &&
    Boolean(
      first.taxiway && second.taxiway && first.taxiway === second.taxiway,
    );
  const sharedApron =
    bothActiveMovers && first.taxiway === "APRON" && second.taxiway === "APRON";
  const requiredHorizontal = Math.max(
    SURFACE_GAP,
    first.bodyRadius + second.bodyRadius + SURFACE_GAP,
  );
  ...
  if (
    includeOperationalSurfaceSeparation &&
    (sameRunway || sameTaxiway || sharedApron) &&
    horizontalDistance < requiredHorizontal
  ) {
    return {
...
export function detectCommittedRunwaySweepConflict(
  ...
  const horizontalDistance = Math.hypot(
    surface.x - trajectory.x,
    surface.y - trajectory.y,
  );
  const requiredHorizontal =
    surface.bodyRadius + trajectory.bodyRadius + PHYSICAL_GAP;
...
export function detectCommittedRunwaySweepSegmentConflict(
  ...
  if (!surface.surface || (!from.protectedSurface && !to.protectedSurface))
    return null;
  ...
  const amount =
    lengthSquared <= 1e-12
      ? 0
      : clamp(
          ((surface.x - from.x) * dx + (surface.y - from.y) * dy) /
            lengthSquared,
          0,
          1,
        );
  ...
  const requiredHorizontal =
    surface.bodyRadius +
    Math.max(from.bodyRadius, to.bodyRadius) +
    PHYSICAL_GAP;
  if (horizontalDistance >= requiredHorizontal) return null;
  return {
    type: "runway-incursion",
    ...
    requiredHorizontal,
    detail:
      "surface aircraft would enter the continuous committed runway wing sweep",
  };
}
...
export function detectAircraftObstacleConflict(
  ...
  const verticalOverlap =
    aircraft.minimumAltitude < obstacle.maximumAltitude &&
  ...
  if (
    ...
    return null;
  const horizontalDistance = distanceToObstacleBoundary(
...
export function findFlightConflicts(
  config: AirportConfig,
  flights: Flight[],
): FlightConflict[] {
  const proxies = flights.map((flight) => ({
    flight,
    proxy: flightProxy(config, flight),
  }));
  const conflicts: FlightConflict[] = [];
  for (let firstIndex = 0; firstIndex < proxies.length; firstIndex += 1) {
    for (
      let secondIndex = firstIndex + 1;
      secondIndex < proxies.length;
      secondIndex += 1
    ) {
...
export function findObstacleConflicts(
  config: AirportConfig,
  flights: Flight[],
): AircraftObstacleConflict[] {
...
function nearbyObstacles(
  ...
  const minimumCellX = Math.floor(
    (aircraft.x - searchRadius) / OBSTACLE_GRID_CELL_SIZE,
  );
  const maximumCellX = Math.floor(
    (aircraft.x + searchRadius) / OBSTACLE_GRID_CELL_SIZE,
  );
  const minimumCellY = Math.floor(
    (aircraft.y - searchRadius) / OBSTACLE_GRID_CELL_SIZE,
  );
  ...
  for (let cellX = minimumCellX; cellX <= maximumCellX; cellX += 1) {
    for (let cellY = minimumCellY; cellY <= maximumCellY; cellY += 1) {
      for (const obstacle of index.cells.get(`${cellX}:${cellY}`) ?? [])
...
export function findProposedConflict(
  ...
  if (mergeWinnerId !== undefined) {
    ...
    if (winner) {
      const winnerProgress =
        proposedProgressById.get(winner.id) ?? winner.progress;
      const winnerEnvelope = aircraftCollisionEnvelope(
        config,
        winner,
        winnerProgress,
      );
      return {
        ...
        horizontalDistance: Math.hypot(
          proposed.x - winnerEnvelope.x,
          proposed.y - winnerEnvelope.y,
        ),
        verticalDistance: Math.abs(proposed.altitude - winnerEnvelope.altitude),
        requiredHorizontal:
          proposed.bodyRadius + winnerEnvelope.bodyRadius + SURFACE_GAP,
  ...
  for (const obstacle of nearbyObstacles(config, proposed)) {
    if (collisionPerformanceTrace)
      collisionPerformanceTrace.obstacleCandidateChecks += 1;
    ...
    if (
      existing &&
      conflict.horizontalDistance > existing.horizontalDistance + 1e-6
    )
      continue;
    return conflict;
  }
  ...
  if (flight.phase === "taxi-in" || flight.phase === "taxi-out") {
    for (const other of otherFlights) {
      if (other.id === flight.id || !flightHasCommittedRunwayTrajectory(other))
        continue;
      ...
      for (const protectedFutureEnvelope of protectedSweep.envelopes) {
        if (collisionPerformanceTrace)
          collisionPerformanceTrace.committedSweepEnvelopeChecks += 1;
        ...
        const pairRadius = Math.max(
          AIR_SURFACE_HORIZONTAL,
          proposed.bodyRadius +
            protectedFutureEnvelope.bodyRadius +
            PHYSICAL_GAP,
        );
        if (
        ...
        const sweepConflict = detectCommittedRunwaySweepConflict(
          proposed,
          protectedFutureEnvelope,
        );
  ...
  for (const other of otherFlights) {
    ...
    const committedTrajectory =
      flight.phase === "approach" ||
      flight.phase === "landing" ||
      flight.phase === "takeoff";
    const otherIsSurfaceMovement =
      other.phase === "taxi-in" || other.phase === "taxi-out";
    ...
    const currentOther = aircraftCollisionEnvelope(
      config,
      other,
      other.progress,
    );
    ...
    const currentDistance = Math.hypot(
      current.x - currentOther.x,
      current.y - currentOther.y,
    );
    const distanceToCurrentOther = Math.hypot(
      proposed.x - currentOther.x,
      proposed.y - currentOther.y,
    );
    const physicallyClear =
      conflict.horizontalDistance >=
    ...
    if (!otherIsMoving) {
      ...
      if (existingConflict && distanceToCurrentOther > currentDistance + 1e-6)
        continue;
      return conflict;
    }
    if (resolvedFlightIds?.has(other.id)) {
      if (
        existingConflict &&
        conflict.horizontalDistance > existingConflict.horizontalDistance + 1e-6
      )
        continue;
      return conflict;
    }
    ...
    if (existingConflict && distanceToCurrentOther <= currentDistance + 1e-6)
      return conflict;
    if (!existingConflict && distanceToCurrentOther < currentDistance)
      return conflict;
  }
  return null;
}
 ...
 */
function committedRunwaySweep(
  config: AirportConfig,
  flight: Flight,
): CommittedRunwaySweep {
  if (collisionPerformanceTrace)
    collisionPerformanceTrace.committedSweepRequests += 1;
  ...
  const progressBucket = Math.floor(
    clamp(flight.progress, 0, 1) * COMMITTED_SWEEP_PROGRESS_BUCKETS,
  );
  const key = [
    flight.phase,
    progressBucket,
  ...
  if (cached?.key === key) {
    if (collisionPerformanceTrace)
      collisionPerformanceTrace.committedSweepCacheHits += 1;
    return cached.sweep;
  }
  if (collisionPerformanceTrace)
    collisionPerformanceTrace.committedSweepBuilds += 1;
  const landingPreview: Flight | undefined =
    flight.phase === "approach"
      ? {
          ...flight,
          phase: "landing",
      ...
      : undefined;
  if (landingPreview)
    landingPreview.motion = sampleFlightMotion(config, landingPreview, 0);
  ...
  for (const trajectory of landingPreview
    ? [flight, landingPreview]
    : [flight]) {
    const startProgress = trajectory === flight ? flight.progress : 0;
    for (
      let sampleIndex = 0;
      sampleIndex <= COMMITTED_SWEEP_SEGMENTS;
      sampleIndex += 1
    ) {
      if (collisionPerformanceTrace)
        collisionPerformanceTrace.committedSweepEnvelopeSamples += 1;
      const futureProgress =
        startProgress +
        ((1 - startProgress) * sampleIndex) / COMMITTED_SWEEP_SEGMENTS;
      ...
      const protectedEnvelope = {
        ...envelope,
        bodyRadius: envelope.bodyRadius + controllerBuffer,
      };
...
function surfaceMergeEscapeIsClear(
  ...
  if (collisionPerformanceTrace)
    collisionPerformanceTrace.mergeEscapeCalls += 1;
  if (
  ...
  const endProgress = Math.min(
    1,
    Math.max(proposedProgress, flight.progress + 240 / routeDistanceM),
  );
  ...
  for (let index = 1; index <= samples; index += 1) {
    if (collisionPerformanceTrace)
      collisionPerformanceTrace.mergeEscapeEnvelopeSamples += 1;
    const progress =
      proposedProgress + ((endProgress - proposedProgress) * index) / samples;
    const candidate = aircraftCollisionEnvelope(config, flight, progress);
    if (candidate.protectedSurface) return false;
    if (candidate.taxiway && candidate.taxiway !== current.taxiway)
      leftSharedTaxiway = true;
    const distance = Math.hypot(
      candidate.x - stationary.x,
      candidate.y - stationary.y,
    );
    const verticalOverlap =
      candidate.minimumAltitude < stationary.maximumAltitude &&
      candidate.maximumAltitude > stationary.minimumAltitude;
    if (
      verticalOverlap &&
      distance < candidate.bodyRadius + stationary.bodyRadius + PHYSICAL_GAP
    )
      return false;
