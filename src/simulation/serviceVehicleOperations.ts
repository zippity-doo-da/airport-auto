...
const serviceVehicleRouteGeometryCache = new WeakMap<
  ServiceVehicleState,
  ServiceVehicleRouteGeometry
>();
...
export function createServiceVehiclePlans(
  config: AirportConfig,
  flight: Flight,
  actualGateInSeconds: number,
  reservedDepotNodeIds: ReadonlySet<string> = new Set(),
  planning?: SurfaceRoutePlanning,
): ServiceVehicleState[] {
  const stand = config.surfaceGraph.stands.find(
    (candidate) => candidate.id === flight.standId,
  );
  if (!stand) return [];
  ...
  return flight.turnaround.tasks
    ...
    .flatMap((task) => {
      ...
      if (!spec) return [];
      const routes = serviceDepotRoutes(
        config.surfaceGraph,
        stand,
        routePool,
        task.type,
        flight.id,
        allocatedDepotNodeIds,
        presentationScale(config) * 1.8,
      );
      allocatedDepotNodeIds.add(routes.depotNodeId);
      const standPath = serviceStandPath(config, flight, stand, spec);
      const outboundTravelSeconds =
        (routes.outbound.distance * WORLD_METERS_PER_UNIT) /
        spec.maximumSpeedMps;
      const localTravelSeconds =
        (polylineLength(standPath) * WORLD_METERS_PER_UNIT) /
        spec.maximumSpeedMps;
      ...
      const dispatchAtSeconds =
        actualGateInSeconds +
        task.scheduledStartOffsetSeconds -
        outboundTravelSeconds -
        localTravelSeconds;
      const depot =
        surfaceNodeIndex(config.surfaceGraph).get(routes.depotNodeId)
          ?.position ?? standPath[0];
...
/** Pre-position a medical responder at the assigned stand without using runway pavement. */
export function createMedicalResponseVehiclePlan(
  config: AirportConfig,
  flight: Flight,
  dispatchAtSeconds: number,
  reservedDepotNodeIds: ReadonlySet<string> = new Set(),
  planning?: SurfaceRoutePlanning,
): ServiceVehicleState | null {
  const stand = config.surfaceGraph.stands.find(
    (candidate) => candidate.id === flight.standId,
  );
  ...
  const routes = serviceDepotRoutes(
    ...
    "maintenance",
    flight.id,
    reservedDepotNodeIds,
    presentationScale(config) * 1.8,
  );
  const standPath = serviceStandPath(config, flight, stand, spec);
  const depot =
    surfaceNodeIndex(config.surfaceGraph).get(routes.depotNodeId)?.position ??
    standPath[0];
  return {
    id: `EV-${flight.id}-medical`,
    ...
    protectedMovementArea: false,
    protectedMovementAuthorized: false,
    emergencyServiceDurationSeconds: 45,
  };
}

export function vehicleSpecForService(
  service: TurnaroundServiceType,
  flight: Flight,
): VehicleSpec | undefined {
  if (
    service === "boarding" &&
    flight.gateAssignment?.serviceArea !== "passenger-terminal"
  ) {
    return {
      type: "passenger-bus",
...
export function ungatedServiceTypes(
  flight: Flight,
  vehicles: ServiceVehicleState[],
): Set<TurnaroundServiceType> {
  const vehicleServices = new Set(vehicles.map((vehicle) => vehicle.service));
  return new Set(
    flight.turnaround.tasks
      // A missing vehicle does not mean that a vehicle-backed task is free.
      // Medical arrivals deliberately begin their turnaround while the
      // ambulance owns the stand lane; those tasks must remain waiting until
      // their physical fleet is released after the transfer. Only genuinely
      // terminal-only tasks (such as boarding) can proceed without a vehicle.
      .filter(
        (task) =>
          task.required &&
          !vehicleServices.has(task.type) &&
          !vehicleSpecForService(task.type, flight),
      )
      .map((task) => task.type),
  );
}

export function availableVehicleServices(
  flight: Flight,
  vehicles: ServiceVehicleState[],
): Set<TurnaroundServiceType> {
  const turnaroundVehicles = vehicles.filter(
    (vehicle) => !vehicle.emergencyResponseKind,
  );
  const available = ungatedServiceTypes(flight, turnaroundVehicles);
  for (const vehicle of turnaroundVehicles) {
...
export function serviceVehiclesBlockingPushback(
  vehicles: ServiceVehicleState[],
  flightId: number,
): ServiceVehicleState[] {
 ...
 */
export function serviceVehicleReservationClaims(
  ...
  if (vehicle.status === "dispatching" || vehicle.status === "returning") {
    ...
    if (graphProgress === null) {
      const boundaryDistance = serviceVehicleGraphBoundaryDistance(
        graph,
        vehicle,
      );
      const boundaryEdgeId =
        vehicle.status === "dispatching"
      ...
      if (boundaryDistance <= SERVICE_GRAPH_BOUNDARY_RESERVATION_WORLD) {
        claims.unshift({
          ...
          id:
            vehicle.status === "dispatching"
              ? (vehicle.outboundRoute[vehicle.outboundRoute.length - 1] ??
                vehicle.depotNodeId)
              : (vehicle.returnRoute[0] ?? vehicle.depotNodeId),
      ...
      if (
        boundaryEdge &&
        boundaryDistance <= SERVICE_GRAPH_BOUNDARY_RESERVATION_WORLD
      ) {
        claims.push({
          kind: "edge",
          id: boundaryEdge.id,
    ...
    return (
      surfaceRouteReservationClaims(
        ...
        .map((claim) =>
          claim.kind === "edge" || claim.kind === "alley"
            ? { ...claim, direction: serviceVehicleOwnerId(vehicle) }
            : claim,
        )
    );
  }
...
export function setServiceVehicleStatus(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
...
export function advanceServiceVehicleMotion(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
  deltaSeconds: number,
): boolean {
  ...
  const acceleration =
    vehicle.groundSpeedMps < vehicle.maximumSpeedMps ? 1.8 : -2.2;
  vehicle.groundSpeedMps = clamp(
    vehicle.groundSpeedMps + acceleration * deltaSeconds,
    0,
    vehicle.maximumSpeedMps,
  );
  const remainingM = Math.max(
    0,
    (1 - vehicle.progress) * distanceWorld * WORLD_METERS_PER_UNIT,
  );
  ...
  vehicle.progress = clamp(
    vehicle.progress +
      (vehicle.groundSpeedMps * deltaSeconds) /
        (distanceWorld * WORLD_METERS_PER_UNIT),
    0,
    1,
  );
...
export function syncServiceVehiclePose(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): void {
  ...
  if (vehicle.status === "dispatching") {
    sample = samplePolyline(
      serviceVehicleTravelPoints(graph, vehicle),
      vehicle.progress,
    );
    const graphProgress = serviceVehicleGraphProgress(graph, vehicle);
    const graphSample =
  ...
  vehicle.protectedMovementArea = Boolean(
    vehicle.currentEdge &&
    serviceVehicleProtectedEdgeIds(graph).has(vehicle.currentEdge),
  );
}

export function serviceVehicleProtectedEdgeIds(
  graph: AirportSurfaceGraph,
): Set<string> {
  return new Set(
    graph.edges
      .filter(
        (edge) =>
          ...
          Boolean(edge.crossedRunwayIds?.length),
      )
      .map((edge) => edge.id),
  );
}

export function serviceVehicleRouteViolations(
  graph: AirportSurfaceGraph,
  vehicles: ServiceVehicleState[],
): Array<{ vehicle: string; edge: string }> {
  const protectedEdges = serviceVehicleProtectedEdgeIds(graph);
  return vehicles.flatMap((vehicle) =>
    [...new Set([...vehicle.outboundRouteEdges, ...vehicle.returnRouteEdges])]
      .filter(
        (edge) =>
          protectedEdges.has(edge) && !vehicle.protectedMovementAuthorized,
      )
      .map((edge) => ({ vehicle: vehicle.id, edge })),
  );
}

export function findServiceVehicleConflicts(
  config: AirportConfig,
  vehicles: ServiceVehicleState[],
  flights: Flight[],
): ServiceVehicleConflict[] {
  ...
  for (let firstIndex = 0; firstIndex < active.length; firstIndex += 1) {
    ...
    for (
      ...
      const requiredDistance =
        serviceVehicleRadius(config, first) +
        serviceVehicleRadius(config, second);
    ...
    for (const flight of flights) {
      ...
      const requiredDistance =
        serviceVehicleRadius(config, first) + aircraft.bodyRadius;
      if (distance + 1e-6 < requiredDistance)
        conflicts.push({
...
export function serviceVehicleRadius(
  config: Pick<AirportConfig, "scope">,
  vehicle: Pick<ServiceVehicleState, "type">,
): number {
  const large =
    vehicle.type === "passenger-bus" ||
    vehicle.type === "baggage-cart" ||
    vehicle.type === "cargo-loader";
...
function serviceDepotRoutePool(
  ...
  const blockedEdgeIds = new Set([
    ...serviceVehicleProtectedEdgeIds(graph),
    ...(planning?.blockedEdgeIds ?? []),
  ]);
  ...
  const zoneEdges = (zone?.edgeIds ?? [])
    .map((edgeId) => edgeById.get(edgeId))
    .filter((edge) => edge && !blockedEdgeIds.has(edge.id));
  const nearbyEdges = graph.edges.filter((edge) => {
    ...
    return (
      Math.min(
        distance(from.position, stand.position),
        distance(to.position, stand.position),
      ) <= 16
    );
  });
  const candidateIds = [
    ...new Set(
      [...zoneEdges, ...nearbyEdges].flatMap((edge) =>
    ...
    .sort((firstId, secondId) => {
      ...
      return (
        distance(first, stand.position) - distance(second, stand.position) ||
        firstId.localeCompare(secondId)
      );
    })
    .slice(0, SERVICE_DEPOT_CANDIDATE_LIMIT);
  const routePlanning: SurfaceRoutePlanning = {
    edgePenaltyById: planning?.edgePenaltyById,
    blockedEdgeIds,
  };
  const routesByDepot = findSurfaceHubRoutes(
    graph,
    stand.rampNodeId,
    candidateIds,
    undefined,
  ...
  return candidateIds.flatMap((nodeId) => {
    ...
    return routes
      ? [{ nodeId, outbound: routes.toHub, returning: routes.fromHub }]
      : [];
  });
}
...
function serviceDepotRoutes(
  ...
  const targetDistance =
    2.8 + positiveModulo(stringHash(`${service}:${flightId}`), 6) * 0.62;
  const candidates = routePool
    .filter((candidate) => {
      ...
      for (const reservedNodeId of reservedDepotNodeIds) {
        ...
        if (
          reservedPosition &&
          distance(position, reservedPosition) < minimumDepotSeparation
        )
          return false;
      }
      return true;
    ...
    .map((candidate) => ({
      ...candidate,
      score:
        Math.abs(candidate.outbound.distance - targetDistance) +
        candidate.returning.distance * 0.08 +
        positiveModulo(stringHash(`${service}:${candidate.nodeId}`), 101) /
          1_000,
    }))
    .sort(
      (first, second) =>
        first.score - second.score || first.nodeId.localeCompare(second.nodeId),
    );
...
function serviceStandPath(
  ...
  const lateral = Math.max(
    profile.visual.bodyRadius * scale + scale * 0.56,
    halfSpan * 0.52,
  );
  ...
  const staging = add(
    add(
      stand.position,
      forward,
      -(halfLength + scale * (1.2 + spec.stagingOffset * 1.45)),
    ),
    left,
    side * (lateral + scale * (1.35 + spec.stagingOffset * 0.28)),
  );
  const bend = add(
    add(stand.position, forward, spec.longitudinal * halfLength),
    left,
    side * (lateral + scale * 2),
  );
  const bay = add(
    add(stand.position, forward, spec.longitudinal * halfLength),
    left,
...
function serviceVehicleStageDistance(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): number {
  if (vehicle.status === "dispatching" || vehicle.status === "returning") {
    ...
    return vehicle.status === "returning"
      ? geometry.returnDistance
      : geometry.dispatchDistance;
  }
  if (vehicle.status === "approaching" || vehicle.status === "clearing")
    return polylineLength(vehicle.standPath);
  return 0;
}

function serviceVehicleTravelPoints(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): Point[] {
  const geometry = serviceVehicleRouteGeometry(graph, vehicle);
  return vehicle.status === "returning"
    ? geometry.returnPoints
    : geometry.dispatchPoints;
}

/** Null means the vehicle is on the stand-to-graph staging connector. */
function serviceVehicleGraphProgress(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): number | null {
  const geometry = serviceVehicleRouteGeometry(graph, vehicle);
  const graphDistance =
    vehicle.status === "returning"
      ? geometry.returnGraphDistance
      : geometry.outboundGraphDistance;
  const totalDistance =
    vehicle.status === "returning"
      ? geometry.returnDistance
      : geometry.dispatchDistance;
...
function serviceVehicleGraphBoundaryDistance(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): number {
  const geometry = serviceVehicleRouteGeometry(graph, vehicle);
  if (vehicle.status === "returning") {
    const connectorDistance = Math.max(
      0,
      geometry.returnDistance - geometry.returnGraphDistance,
    );
...
function serviceVehicleRouteGeometry(
  graph: AirportSurfaceGraph,
  vehicle: ServiceVehicleState,
): ServiceVehicleRouteGeometry {
...
function dedupePoints(points: Point[]): Point[] {
  return points.filter(
    (point, index) => index === 0 || distance(point, points[index - 1]) > 1e-5,
  );
}
...
function polylineLength(points: Point[]): number {
  return points
    .slice(0, -1)
    .reduce(
      (total, point, index) => total + distance(point, points[index + 1]),
      0,
    );
}
