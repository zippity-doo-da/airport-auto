...
type UserSurfaceDisruptionKind = Exclude<SurfaceDisruptionKind, 'disabled-aircraft'>;
...
export function updateSurfaceDisruptionTargetOptions(
  ...
  kindElement: HTMLSelectElement,
  targetElement: HTMLSelectElement,
): void {
  const previous = targetElement.value;
  const kind = kindElement.value as UserSurfaceDisruptionKind;
  ...
  targetElement.replaceChildren(...targets.map((target) => {
    const option = document.createElement('option');
    option.value = target.value;
    option.textContent = target.label;
    return option;
  }));
  if (targets.some((target) => target.value === previous)) targetElement.value = previous;
}
...
export function surfaceDisruptionPanelKey(
  ...
  return [
    ...
    ...state.disruptions.map((disruption) => [
      ...
      Math.max(0, Math.ceil((disruption.expectedClearAtSeconds ?? state.elapsed) - state.elapsed)),
...
export function renderSurfaceDisruptionPanel(
  ...
  elements.apply.disabled = state.replayMode || !supervisor || !elements.target.value;
  elements.target.disabled = state.replayMode || !supervisor;
  elements.kind.disabled = state.replayMode || !supervisor;
  elements.duration.disabled = state.replayMode || !supervisor;
  ...
  const rows = state.disruptions.map((disruption) => {
    ...
    if (disruption.kind === 'disabled-aircraft') {
      ...
      button.disabled = state.replayMode || disruption.status === 'recovering' || !state.canRecover;
    } else {
      button.textContent = 'Reopen';
      button.dataset.clearDisruption = disruption.id;
      button.disabled = state.replayMode || !supervisor;
    }
