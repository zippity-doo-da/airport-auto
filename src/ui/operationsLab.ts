...
export function createOperationsLab(
  ...
  const elements: Elements = {
    ...
    flowCauseList: required(panel, "[data-operations-flow-causes]"),
  ...
  function render(snapshot: OperationsAnalyticsSnapshot): void {
    ...
    elements.summary.replaceChildren(
      metric(
        "Flow",
        `${snapshot.summary.movementsPerHour.toFixed(1)}/hr`,
        `${snapshot.summary.arrivals} in · ${snapshot.summary.departures} out`,
      ),
      metric(
        "Traffic",
        String(snapshot.summary.activeFlights),
        `${snapshot.summary.observedFlights} observed`,
      ),
      metric(
        "Longest queue",
        formatDuration(snapshot.summary.longestQueueSeconds),
        `${snapshot.queueUtilization.filter((entry) => entry.averageEntries > 0).length} active categories`,
      ),
      metric(
        "Safety picture",
      ...
      metric(
        "Fuel",
        `${Math.round(snapshot.summary.fuelBurnKg).toLocaleString()} kg`,
        `${Math.round(snapshot.summary.holdingFuelBurnKg).toLocaleString()} kg holding`,
      ),
      metric(
        "Slot revisions",
    ...
    const nextOptionKey = snapshot.flights
      .map((flight) => `${flight.flightId}:${flight.callsign}`)
      .join("|");
    if (nextOptionKey !== optionKey) {
      ...
      selectedFlightId =
        previousValue !== null &&
        snapshot.flights.some((flight) => flight.flightId === previousValue)
          ? previousValue
    ...
    const samples =
      snapshot.selectedFlightId === selectedFlightId
    ...
    const altitudeMaximum = Math.max(
      1_000,
      ...chartSamples.map((sample) => sample.altitudeFt),
    );
    const speedMaximum = Math.max(
      100,
      ...chartSamples.map((sample) =>
        Math.max(sample.airspeedKts, sample.groundSpeedKts),
      ),
    );
    elements.altitudePath.setAttribute(
      "d",
      linePath(
        chartSamples.map((sample) => sample.altitudeFt),
        360,
    ...
    elements.speedPath.setAttribute(
      "d",
      linePath(
        chartSamples.map((sample) =>
          Math.max(sample.airspeedKts, sample.groundSpeedKts),
    ...
    elements.fuelPath.setAttribute(
      "d",
      linePath(
        chartSamples.map((sample) => sample.fuelPercent),
        360,
        92,
        0,
        100,
    ...
    const maximumRunwaySeconds = Math.max(
      1,
      ...snapshot.runwayUtilization.map((entry) => entry.occupiedSeconds),
    );
    elements.runwayList.replaceChildren(
      ...snapshot.runwayUtilization.map((entry) =>
        utilizationRow(
          entry.label,
    ...
    const maximumTaxiwaySeconds = Math.max(
      1,
      ...visibleTaxiways.map((entry) => entry.occupiedSeconds),
    );
    elements.taxiwayList.replaceChildren(
      ...(visibleTaxiways.length
        ? visibleTaxiways.map((entry) =>
            utilizationRow(
              entry.label,
              entry.occupiedSeconds,
        ...
        : [
            Object.assign(document.createElement("p"), {
              textContent:
                "Taxiway observations will appear as surface traffic moves.",
            }),
          ]),
    );
    ...
    const maximumCauseCount = Math.max(
      1,
      ...snapshot.trafficFlowCauses.map(
        (entry) => entry.revisions + entry.initialAssignments,
      ),
    );
    elements.flowCauseList.replaceChildren(
      ...(snapshot.trafficFlowCauses.length
        ? snapshot.trafficFlowCauses.map((entry) => {
            const row = utilizationRow(
              entry.category.replaceAll("-", " "),
              entry.largestAbsoluteShiftSeconds,
              ((entry.revisions + entry.initialAssignments) /
                maximumCauseCount) *
                100,
              `${entry.revisions} change${entry.revisions === 1 ? "" : "s"} · ${entry.initialAssignments} initial`,
            );
            row.className = "operations-lab__flow-row";
            const detail = document.createElement("p");
            detail.textContent = `+${formatDuration(entry.delayAddedSeconds)} delay · −${formatDuration(entry.delayRecoveredSeconds)} recovered · ${entry.latestReason}`;
            detail.title = entry.latestReason;
            row.append(detail);
            return row;
          })
        : [
            Object.assign(document.createElement("p"), {
              textContent:
                "Meter-slot revisions will appear when demand, weather, gates, or runway conditions move a planned release.",
            }),
          ]),
    );
...
function renderHeatmap(
  elements: Elements,
  snapshot: OperationsAnalyticsSnapshot,
): void {
  ...
  const maximumCount = Math.max(
    1,
    ...snapshot.conflictHeatmap.map((cell) => cell.count),
  );
  for (const cell of snapshot.conflictHeatmap) {
    ...
    circle.setAttribute(
      "class",
      cell.warningCount > 0
        ? "operations-lab__heat-warning"
        : "operations-lab__heat-caution",
    );
    circle.setAttribute(
      "aria-label",
      `${cell.count} conflict forecasts near ${Math.round(cell.x)}, ${Math.round(cell.z)}`,
    );
    nodes.push(circle);
  }
