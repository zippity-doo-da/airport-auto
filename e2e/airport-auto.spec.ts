...
test("Assisted ORD shift exposes proposals, station workload, and structured control", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "The focused mobile Watch test covers the responsive controls and radar inset.",
  );
  ...
  await page.goto(
    "/?airport=ORD&mode=assisted&station=supervisor&autostart=1&detail=low&renderFps=0.25",
  );
  ...
  await expect(page.locator(".flight-chip__route").first()).toContainText(
    / · .+, .+ → .+ · .+, .+/,
  );

  const initial = await page.evaluate(() => window.airportControl.snapshot());
  expect(initial.schemaVersion).toBe(42);
  ...
  expect(initial.airportLifeVisible).toBeFalsy();
  expect(initial.flights.length).toBeGreaterThan(0);
  const aircraftFlight = initial.flights[0];
  expect(aircraftFlight.operationalDetail).toMatchObject({
    schemaVersion: 1,
    kind: expect.any(String),
    label: expect.any(String),
    reason: expect.any(String),
    maintenanceClass: expect.any(String),
    airworthinessStatus: expect.any(String),
  });
  expect(aircraftFlight.aircraft).toMatchObject({
    manufacturer: expect.any(String),
    fleetRole: expect.any(String),
    visualFamily: expect.any(String),
    lengthM: expect.any(Number),
    wingspanM: expect.any(Number),
    maxTakeoffWeightT: expect.any(Number),
    maxLandingWeightT: expect.any(Number),
    approachKts: expect.any(Number),
    rotationKts: expect.any(Number),
    takeoffRunwayRequiredM: expect.any(Number),
    landingRunwayRequiredM: expect.any(Number),
    serviceMinutes: expect.any(Number),
    auditedOn: expect.stringMatching(/^\d{4}-\d{2}-\d{2}$/),
  });
  expect(aircraftFlight.aircraft.dataReferenceIds.length).toBeGreaterThan(0);
  expect(aircraftFlight.systems).toMatchObject({
    schemaVersion: 1,
    gearExtension: expect.any(Number),
    flapExtension: expect.any(Number),
    slatExtension: expect.any(Number),
    spoilerExtension: expect.any(Number),
    reverserExtension: expect.any(Number),
    lights: {
      navigation: expect.any(Boolean),
      beacon: expect.any(Boolean),
      strobe: expect.any(Boolean),
      landing: expect.any(Boolean),
      taxi: expect.any(Boolean),
      recognition: expect.any(Boolean),
    },
    effects: {
      exhaust: expect.any(Number),
      condensation: expect.any(Number),
      tireSmoke: expect.any(Number),
      surfaceSpray: expect.any(Number),
    },
  });
  for (const value of [
    aircraftFlight.systems.gearExtension,
    aircraftFlight.systems.flapExtension,
    aircraftFlight.systems.slatExtension,
    aircraftFlight.systems.spoilerExtension,
    aircraftFlight.systems.reverserExtension,
    aircraftFlight.systems.effects.exhaust,
    aircraftFlight.systems.effects.condensation,
    aircraftFlight.systems.effects.tireSmoke,
    aircraftFlight.systems.effects.surfaceSpray,
  ]) {
    expect(value).toBeGreaterThanOrEqual(0);
    expect(value).toBeLessThanOrEqual(1);
  }
  expect(initial.renderer.aircraftAssets.active).toBeGreaterThan(0);
  expect(
    Object.keys(initial.renderer.aircraftAssets.families).length,
  ).toBeGreaterThan(0);
  expect(initial.renderer.aircraftAssets).toMatchObject({
    poolBudget: 24,
    maximumTexturesPerAircraft: 0,
  });
  expect(
    initial.renderer.aircraftAssets.maximumMeshesPerAircraft,
  ).toBeLessThanOrEqual(46);
  expect(
    initial.renderer.aircraftAssets.maximumMaterialsPerAircraft,
  ).toBeLessThanOrEqual(28);
  expect(
    initial.renderer.aircraftAssets.maximumTrianglesPerAircraft,
  ).toBeLessThanOrEqual(2_100);
  expect(
    initial.renderer.aircraftAssets.maximumGeometryBytesPerAircraft,
  ).toBeLessThanOrEqual(57_344);
  const airportLifeEnabled = await page.evaluate(() =>
    window.airportControl.request({
      action: "setAirportLifeVisible",
      enabled: true,
    }),
  );
  expect(airportLifeEnabled).toMatchObject({
    accepted: true,
    reason: "accepted",
  });
  expect(airportLifeEnabled.resultingState.airportLifeVisible).toBeTruthy();
  await expect(page.locator("#airport-life-toggle")).toBeChecked();
  await expect(
    page
      .locator(`[data-flight-chip="${aircraftFlight.id}"] .flight-chip__detail`)
      .first(),
  ).toContainText(aircraftFlight.operationalDetail.label);
  const airportLifeDisabled = await page.evaluate(() =>
    window.airportControl.request({
      action: "setAirportLifeVisible",
      enabled: false,
    }),
  );
  expect(airportLifeDisabled.accepted).toBeTruthy();
  expect(airportLifeDisabled.resultingState.airportLifeVisible).toBeFalsy();
  await expect(page.locator("#airport-life-toggle")).not.toBeChecked();
  expect(initial.remoteControl).toMatchObject({
    schemaVersion: 1,
    configured: false,
    status: "disconnected",
    connected: false,
  ...
  expect(JSON.stringify(initial.remoteControl)).not.toContain("token");
  await expect(page.locator(".remote-control-host")).toBeAttached();
  await expect(page.locator("#remote-host-endpoint")).toHaveValue("");
  await expect(page.locator("#remote-host-token")).toHaveValue("");
  await expect(page.locator("#remote-host-disconnect")).toBeDisabled();
  const remoteProbe = await page.evaluate(async () => {
    let rejection = "";
    try {
      await window.airportControl.remote.connect({
        endpoint: "ws://remote.example/v1/ws",
        sessionId: "e2e-session",
        token: "this-is-a-long-enough-e2e-placeholder-token",
      });
    } catch (error) {
    ...
    return {
      rejection,
      state: window.airportControl.remote.state(),
      help: window.airportControl.help().remoteConnect,
    };
  });
  expect(remoteProbe.rejection).toContain("wss://");
  expect(remoteProbe.state).toMatchObject({
    configured: false,
    connected: false,
  });
  expect(remoteProbe.help).toContain("explicit opt-in");
  const protocolProbe = await page.evaluate(() => {
    const protocol = window.airportControl.protocol();
    const invalid = window.airportControl.validate({
      action: "pause",
      unknown: true,
    });
    const circularCommand: Record<string, unknown> = { action: "pause" };
    circularCommand.unknown = circularCommand;
    const circularRejected = (
      window.airportControl.request as unknown as (
        command: unknown,
      ) => ReturnType<typeof window.airportControl.request>
    )(circularCommand);
  ...
  expect(protocolProbe.protocol).toMatchObject({
    ...
    firstCommand: {
      action: "pause",
      authority: { rule: "session", safetyArbiter: true },
    },
  });
  expect(
    protocolProbe.protocol.firstCommand.compatibility.transports,
  ).toContain("websocket");
  expect(protocolProbe.invalid).toMatchObject({
    valid: false,
    action: "pause",
  });
  ...
  expect(protocolProbe.paused.eventKey).toContain(
    `:${protocolProbe.paused.eventId}`,
  );
  expect(protocolProbe.resumed.accepted).toBeTruthy();

  const broadcastResult = await page.evaluate(
    () =>
      new Promise<ReturnType<typeof window.airportControl.request>>(
        (resolve, reject) => {
          ...
          channel.addEventListener("message", (event) => {
            if (
              ...
              event.data?.requestId !== "e2e-broadcast-request"
            )
              return;
  ...
  expect(
    initial.controllers.workloads.map((workload) => workload.station),
  ).toEqual(["approach", "tower", "ground", "ramp"]);
  expect(
    initial.controllers.workloads.every(
      (workload) => workload.responsibilities.length >= 4,
    ),
  ).toBeTruthy();
  expect(
    initial.controllers.performance.map((performance) => performance.station),
  ).toEqual(["supervisor", "approach", "tower", "ground", "ramp"]);
  expect(
    initial.controllers.performance.every(
      (performance) =>
  ...
  expect(
    new Set(
      initial.controllers.performance.map(
        (performance) => performance.trafficScope,
      ),
    ).size,
  ).toBe(5);
  ...
  expect(
    initial.controllers.evaluation.stations.map(
      (evaluation) => evaluation.station,
    ),
  ).toEqual(["supervisor", "approach", "tower", "ground", "ramp"]);
  expect(initial.controllers.evaluation.methodology.safetyPriority).toContain(
    "never",
  );
  await expect(page.locator("#station-briefing")).toBeVisible();
  await expect(page.locator("#station-briefing")).toContainText(
    "Supervisor objectives",
  );
  await expect(
    page.locator(".station-briefing__objectives > span"),
  ).toHaveCount(4);
  ...
  await expect(page.locator(".station-briefing__role")).toContainText(
    "Every active aircraft",
  );
  await page.waitForTimeout(1_200);
  await expect(roleDisclosure).toHaveAttribute("open", "");
  await expect(roleSummary).toBeFocused();
  await page.screenshot({
    path: testInfo.outputPath("controller-supervisor-briefing.png"),
  });
  await roleSummary.click();
  const evaluationDisclosure = page.locator(".controller-evaluation");
  await expect(evaluationDisclosure.locator("summary")).toContainText(
    "not rated",
  );
  ...
  await expect(
    evaluationDisclosure.locator(".controller-evaluation__metrics > span"),
  ).toHaveCount(8);
  await page.screenshot({
    path: testInfo.outputPath("controller-decision-evaluation.png"),
  });
  await evaluationDisclosure.locator("summary").click();
  ...
  expect(initial.airport.airspaceProgram.disclaimer).toContain(
    "never for navigation",
  );
  expect(
    initial.airport.airspaceProgram.sources.every((source) =>
      source.url.startsWith("https://www.faa.gov/"),
    ),
  ).toBeTruthy();
  ...
  expect(initial.operations.profile).toMatchObject({
    airportCode: "ORD",
    archetype: "hub-banked",
    schemaVersion: 1,
  });
  expect(initial.operations.current).toMatchObject({
    periodId: "morning-departure",
  });
  expect(initial.operations.current.localTime).toMatch(/^05:[3-5]\d$/);
  expect(initial.operations.current.mix.departureShare).toBeGreaterThan(
    initial.operations.current.mix.arrivalShare,
  );
  expect(initial.operations.trafficProgram).toMatchObject({
    schemaVersion: 2,
    airportCode: "ORD",
    fidelity: "sourced-airlines-schematic-weights",
  });
  expect(initial.operations.density).toMatchObject({
    id: "realistic",
    holdingCapacity: 4,
  });
  expect(initial.trafficManagement).toMatchObject({
    schemaVersion: 1,
    density: { id: "realistic" },
  });
  expect(
    initial.flights.every(
      (flight) =>
        flight.operationPlan.periodId === initial.operations.current.periodId,
    ),
  ).toBeTruthy();
  expect(
    initial.flights.some(
      (flight) => flight.operationPlan.direction === "arrival",
    ),
  ).toBeTruthy();
  expect(
    initial.flights.some(
      (flight) => flight.operationPlan.direction === "departure",
    ),
  ).toBeTruthy();
  await expect(page.locator("#operation-bank")).toContainText(
    "Morning departure bank · Realistic 1.08× bank",
  );
  expect(
    initial.flights.every(
      (flight) =>
        ...
        flight.flightPlan.procedureProfile.dataVersion ===
          initial.airport.airspaceProgram.dataVersion &&
        flight.navigation.procedureDataVersion ===
          initial.airport.airspaceProgram.dataVersion &&
  ...
  expect(new Set(initial.input.actions.map((action) => action.id)).size).toBe(
    24,
  );
  expect(
    initial.renderer.camera.groundWidth / 2 - initial.renderer.camera.panLimitX,
  ).toBeGreaterThanOrEqual(2_800);
  expect(
    initial.renderer.camera.groundHeight / 2 -
      initial.renderer.camera.panLimitY,
  ).toBeGreaterThanOrEqual(2_100);
  ...
  expect(initial.airport.vectorData?.attribution).toContain(
    "Federal Aviation Administration",
  );
  expect(initial.airport.surfaceData?.counts.stands).toBeGreaterThanOrEqual(24);
  expect(initial.airport.surfaceData?.attribution).toContain(
    "OpenStreetMap contributors",
  );
  expect(initial.airport.contextData?.counts).toMatchObject({
    roads: 6_016,
    rails: 1_127,
    waterways: 35,
    areas: 1_568,
  ...
  expect(initial.airport.contextData?.attribution).toContain(
    "OpenStreetMap contributors",
  );
  ...
  expect(
    initial.surfaceGraph.passengerFacilities
      .filter((facility) => facility.kind === "terminal")
      .map((facility) => facility.terminalId)
      .sort(),
  ).toEqual(["T1", "T2", "T3", "T5"]);
  ...
  expect(initial.renderer.surfaceDisruptions).toEqual({
    total: 0,
    pending: 0,
    active: 0,
    recovering: 0,
  ...
  for (const concourse of ["B", "C", "E", "F", "G", "H", "K", "L", "M"]) {
    expect(
      initial.surfaceGraph.stands.filter(
        (stand) => stand.concourse === concourse,
      ).length,
  ...
  expect(
    initial.surfaceGraph.edges.filter(
      (edge) => edge.gradeSeparation === "bridge",
    ),
  ).toHaveLength(2);
  expect(initial.surfaceGraph.rampControlZones.length).toBeGreaterThanOrEqual(
    16,
  );
  expect(initial.surfaceGraph.standFlows).toHaveLength(
    initial.surfaceGraph.stands.length,
  );
  expect(
    initial.surfaceGraph.standFlows.every(
      (flow) =>
  ...
  expect(initial.surfaceGraph.gatePlanning.factors).toEqual(
  ...
  const zoneKinds = new Set(
    initial.surfaceGraph.zones.map((zone) => zone.kind),
  );
  for (const kind of [
    "terminal-complex",
    "terminal-apron",
  ...
  expect(
    initial.runwayConfigurations.map((configuration) => configuration.id),
  ).toEqual([
    "ORD-WEST-FLOW",
    "ORD-EAST-FLOW",
  ...
  expect(
    initial.runwayConfigurations.every((configuration) =>
      configuration.source?.url.startsWith("https://www.faa.gov/"),
    ),
  ).toBeTruthy();
  for (const runway of initial.runways) {
    const presentation = initial.renderer.runways.find(
      (candidate) => candidate.id === runway.id,
    );
    expect(presentation?.activeEnd).toBe(runway.activeEnd);
    expect(presentation?.markerVisible).toBe(
      !runway.closed && runway.role !== "inactive",
    );
    expect(presentation?.arrivalMarkerVisible).toBe(
      runway.role === "arrival" || runway.role === "mixed",
    );
    expect(presentation?.departureMarkerVisible).toBe(
      runway.role === "departure" || runway.role === "mixed",
    );
  }
  expect(initial.renderer.surfaceLayers).toEqual({
  ...
  expect(initial.renderer.context).toMatchObject({
    status: "loaded",
    roads: 6_016,
    rails: 1_127,
    boundaryRings: 1,
  ...
  expect(
    initial.flights.some((flight) => flight.phase === "approach"),
  ).toBeTruthy();
  expect(
    initial.flights.some(
      (flight) => flight.phase === "taxi-out" || flight.phase === "resting",
    ),
  ).toBeTruthy();
  ...
  expect(
    initial.flights.every(
      (flight) =>
        ...
        flight.gate.assignment.scheduledDepartureSeconds >=
          flight.gate.assignment.scheduledGateInSeconds &&
  ...
  expect(
    initial.flights.every(
      (flight) =>
        ...
        flight.turnaround.targetFuelPercent >=
          flight.turnaround.initialFuelPercent &&
        flight.fuelPlan.schemaVersion === 1 &&
        flight.fuelPlan.usableFuelKg === flight.aircraft.usableFuelKg &&
        Math.max(
          flight.turnaround.initialFuelPercent,
          flight.fuelPlan.departure.dispatchFuelPercent,
        ) === flight.turnaround.targetFuelPercent &&
  ...
  expect(
    initial.flights
      ...
      .every(
        (flight) =>
          flight.kinematics.fuelPercent >= 7 &&
          flight.kinematics.fuelPercent <= 32,
      ),
  ).toBeTruthy();
  expect(
    initial.flights
      .filter((flight) => flight.phase === "approach")
      .every((flight) => flight.turnaround.status === "planned"),
  ).toBeTruthy();
  expect(
    initial.flights
      .filter((flight) => flight.phase === "approach")
  ...
  expect(
    initial.flights.every((flight) => flight.deicing.status === "not-required"),
  ).toBeTruthy();
  expect(initial.serviceVehicles.length).toBeGreaterThanOrEqual(2);
  expect(
    initial.serviceVehicles.every(
      (vehicle) =>
        !vehicle.protectedMovementAuthorized &&
        !vehicle.protectedMovementArea &&
        vehicle.outboundRoute.length > 0 &&
        vehicle.returnRoute.length > 0,
    ),
  ).toBeTruthy();
  expect(
    initial.flights.some(
      (flight) => flight.gate?.assignment?.airlineFit === "preferred",
    ),
  ).toBeTruthy();
  expect(
    initial.flights
      .filter(
        (flight) =>
          flight.airline.code === "UA" && flight.service === "passenger",
      )
      .every((flight) =>
        ["B", "C", "E", "F", "G"].includes(flight.gate?.concourse),
      ),
  ).toBeTruthy();
  expect(
    initial.flights
      .filter(
        (flight) =>
          flight.airline.code === "AA" && flight.service === "passenger",
      )
      .every((flight) => ["G", "H", "K", "L"].includes(flight.gate?.concourse)),
  ).toBeTruthy();
  expect(
    initial.flights
      .filter((flight) => flight.service === "cargo")
      .every(
        (flight) =>
  ...
  await expect(
    page.locator(".flight-chip__detail").filter({ hasText: "planned" }).first(),
  ).toBeVisible();
  await expect(
    page.locator(".flight-chip").first().locator(".flight-chip__metric"),
  ).toHaveCount(4);
  await expect(
    page.locator(".flight-chip").first().locator(".flight-chip__metric").nth(3),
  ).toContainText(/Heading\d{3}° [NSEW]/i);
  const taxiingFlight = initial.flights.find(
  ...
  expect(
    taxiingFlight?.taxiPerformance.minimumRouteWingtipClearanceM,
  ).toBeGreaterThanOrEqual(
    taxiingFlight?.taxiPerformance.requiredWingtipClearanceM ?? Infinity,
  );
  expect(
    taxiingFlight?.taxiPerformance.stoppingDistanceM,
  ).toBeGreaterThanOrEqual(0);
  expect(taxiingFlight?.surfaceRoutePlanning.routingCost).toEqual(
    expect.any(Number),
  );
  expect(taxiingFlight?.groundOperation.flowDirection).toMatch(
    /inbound|outbound/,
  );
  const liveTurn = initial.flights.find(
    (flight) =>
      flight.phase === "resting" && flight.turnaround.status !== "ready",
  );
  expect(liveTurn?.groundOperation).toMatchObject({
    pushbackCleared: false,
    pushbackProgress: 0,
    tugAttached: false,
    engineState: "off",
  });
  expect(["planned", "servicing"]).toContain(liveTurn?.turnaround.status);
  expect(liveTurn?.turnaround.progress).toBeLessThan(1);
  expect(liveTurn?.turnaround.blockingServices.length).toBeGreaterThan(0);
  expect(["left", "right", "straight"]).toContain(
    liveTurn?.groundOperation.pushbackDirection,
  );
  expect(liveTurn?.groundOperation.rampControlZoneId).toMatch(/^RAMP-/);
  expect(liveTurn?.groundOperation.alleyId).toEqual(expect.any(String));
  expect(liveTurn?.gate?.assignment).toMatchObject({
    status: "occupied",
    nextDestination: expect.any(String),
  });
  ...
  expect(initial.queues.counts).toMatchObject({
    gate: expect.any(Number),
    ramp: expect.any(Number),
    taxi: expect.any(Number),
    crossing: expect.any(Number),
    runway: expect.any(Number),
    wake: expect.any(Number),
    weather: expect.any(Number),
    downstream: expect.any(Number),
  });
  const focusCandidate = initial.flights.find(
  ...
  const radarResult = await page.evaluate(() =>
    window.airportControl.request({ action: "setRadarVisible", enabled: true }),
  );
  ...
  await expect(page.locator("#queue-list")).toContainText(
    /runway|rwy|arrival|departure/i,
  );
  ...
  const beforeKeyboardPan = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.locator("#scene").focus();
  // W is a continuous camera key with no matching airport-select option, so
  // this verifies gameplay blocking without invoking native select typeahead.
  ...
  const beforeKeyboardRotate = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera.orbitDegrees,
  );
  await page.keyboard.press("q");
  await page.waitForFunction(
    (orbit) =>
      window.airportControl.snapshot().renderer.camera.orbitDegrees !== orbit,
    beforeKeyboardRotate,
  );
  const afterLeftRotate = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera.orbitDegrees,
  );
  ...
  expect(densityResult.resultingState.trafficManagement.density.id).toBe(
    "busy",
  );
  const rushScenarioResult = await page.evaluate(() =>
    window.airportControl.request({ action: "setScenario", scenario: "rush" }),
  );
  ...
  await page.evaluate(() => {
    for (let index = 0; index < 8; index += 1)
      window.airportControl.command({ action: "zoomOut" });
  ...
  const exploredCamera = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  expect(
    Math.hypot(exploredCamera.focusX, exploredCamera.focusY),
  ).toBeGreaterThan(1_400);
  expect(Math.abs(exploredCamera.focusX)).toBeLessThanOrEqual(
    exploredCamera.panLimitX,
  );
  expect(Math.abs(exploredCamera.focusY)).toBeLessThanOrEqual(
    exploredCamera.panLimitY,
  );
  ...
  const rampStation = await page.evaluate(() =>
    window.airportControl.request({ action: "setStation", station: "ramp" }),
  );
  expect(rampStation.accepted).toBeTruthy();
  expect(rampStation.resultingState.controllers.automation).toEqual({
    approach: true,
    tower: true,
    ground: true,
    ramp: false,
  ...
  await page.waitForFunction(
    (flightId) => {
      ...
      const equipmentClear = !snapshot?.serviceVehicles.some(
        (vehicle) =>
          vehicle.flightId === flightId &&
          ["approaching", "servicing", "clearing"].includes(vehicle.status),
      );
      return (
        flight?.phase === "resting" &&
        flight.turnaround.status === "ready" &&
        flight.groundOperation.label === "Ready push" &&
        equipmentClear
      );
    },
    liveTurn!.id,
  ...
  const readySnapshot = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  const pushReady = readySnapshot.flights.find(
    (flight) => flight.id === liveTurn!.id,
  );
  expect(pushReady?.turnaround).toMatchObject({
    status: "ready",
    progress: 1,
    blockingServices: [],
  });
  ...
  await expect(page.locator(".turnaround-panel__summary")).toContainText(
    "Turnaround 100%",
  );
  await expect(page.locator(".turnaround-panel__tasks li")).toHaveCount(
    pushReady!.turnaround.tasks.filter((task) => task.required).length,
  );
  const pushbackButton = page.locator(
    '#flight-actions button[data-flight-action="pushback"]',
  );
  await expect(pushbackButton).toBeVisible();
  await expect(pushbackButton).toHaveText(
    new RegExp(`Push ${pushReady!.groundOperation.pushbackDirection}`, "i"),
  );
  ...
  await page.waitForFunction((flightId) => {
    ...
    return (
      ...
      flight.groundOperation.tugAttached &&
      flight.groundOperation.engineState === "starting"
    );
  }, pushReady!.id);
  await page.waitForFunction(() => {
  ...
  const pushingSnapshot = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  const pushingFlight = pushingSnapshot.flights.find(
    (flight) => flight.id === pushReady!.id,
  );
  ...
  expect(
    pushingSnapshot.recentEvents.some(
      (event) => event.type === "command:clearPushback" && event.accepted,
    ),
  ).toBeTruthy();
  await page.evaluate(() => {
    for (let index = 0; index < 3; index += 1)
      window.airportControl.command({ action: "nextView" });
    for (let index = 0; index < 10; index += 1)
  ...
  await page.locator("#flight-strip-toggle").click();
  await page.evaluate(() =>
    window.airportControl.command({ action: "resetCamera" }),
  );
  await page.evaluate(() =>
    window.airportControl.request({
      action: "setStation",
  ...
  const runwayPlanResult = await page.evaluate(() =>
    window.airportControl.request({
      action: "setRunwayConfiguration",
      configurationId: "ORD-EAST-FLOW",
    }),
  );
  expect(runwayPlanResult.accepted).toBeTruthy();
  expect(
    runwayPlanResult.resultingState.runwayConfiguration.selectionMode,
  ).toBe("manual");
  expect(
    runwayPlanResult.resultingState.runwayConfiguration.transition?.targetId,
  ).toBe("ORD-EAST-FLOW");
  const automaticPlanResult = await page.evaluate(() =>
    window.airportControl.request({
      action: "setRunwayConfiguration",
  ...
  expect(
    automaticPlanResult.resultingState.runwayConfiguration.selectionMode,
  ).toBe("automatic");
  const importedVectorCounts = await page.evaluate(async () => {
    const [vectorResponse, contextResponse, surfaceResponse] =
      await Promise.all([
    ...
    if (!vectorResponse.ok)
      throw new Error(
        `airport vector request failed: ${vectorResponse.status}`,
      );
    if (!contextResponse.ok)
      throw new Error(
        `airport context request failed: ${contextResponse.status}`,
      );
    if (!surfaceResponse.ok)
      throw new Error(
        `airport surface request failed: ${surfaceResponse.status}`,
      );
  ...
  const proposalButton = page.locator(
    "#clearance-advisor button[data-proposal-id]",
  );
  ...
  const advisorControlStayedMounted = await page.evaluate(async () => {
    ...
    return (
      button ===
      document.querySelector("#clearance-advisor button[data-proposal-id]")
    );
  });
  ...
  expect(recording.sharing).toMatchObject({
  ...
  expect(
    recording.events.every(
      (event) => event.eventKey === `${event.sessionId}:${event.eventId}`,
    ),
  ).toBeTruthy();
  ...
  await expect(page.locator("#runway-configuration-select option")).toHaveCount(
    7,
  );
  ...
  await expect(page.locator("#map-data-version")).toContainText(
    "FAA geometry + OSM surface and surroundings",
  );
  await expect(page.locator("#map-data-attribution")).toContainText(
    "not for navigation",
  );
  await expect(page.locator("#map-data-attribution")).toContainText(
  ...
  await expect(page.locator("#map-facility-source")).toHaveAttribute(
    "href",
    initial.airport.surfaceData?.passengerFacilityReference.url ?? "",
  );
  ...
  await expect(
    page.locator("#surface-disruption-list .surface-disruptions__item"),
  ).toHaveCount(1);
  await page.waitForFunction(() => {
    const snapshot = window.airportControl.snapshot();
    const runwayId = snapshot.surfaceDisruptions[0]?.runwayId;
    return (
      snapshot.renderer.surfaceDisruptions.total === 1 &&
      snapshot.renderer.runways.find((runway) => runway.id === runwayId)
        ?.markerVisible === false
    );
  });
  ...
  expect(restricted.surfaceDisruptions[0]).toMatchObject({
    kind: "runway-closure",
    source: "controller",
  });
  expect(restricted.renderer.surfaceDisruptions.total).toBe(1);
  const restrictedRunway = restricted.runways.find(
    (runway) => runway.id === restricted.surfaceDisruptions[0].runwayId,
  );
  expect(restrictedRunway?.closed).toBeTruthy();
  expect(
    restricted.renderer.runways.find(
      (runway) => runway.id === restrictedRunway?.id,
    )?.markerVisible,
  ).toBeFalsy();
  await page.screenshot({
    path: testInfo.outputPath("surface-restriction.png"),
  });
  await page
    .locator("#surface-disruption-list button[data-clear-disruption]")
    .click();
  await expect(
    page.locator("#surface-disruption-list .surface-disruptions__item"),
  ).toHaveCount(0);
  expect(
    (await page.evaluate(() => window.airportControl.snapshot()))
      .surfaceDisruptions,
  ).toHaveLength(0);
  ...
  expect(
    (await page.evaluate(() => window.airportControl.snapshot())).renderer
      .surfaceLayers.hotspots,
  ).toBeTruthy();
  const boundaryLayer = page.locator(
    'input[data-surface-layer="airport-boundary"]',
  );
  await expect(boundaryLayer).not.toBeChecked();
  await boundaryLayer.check();
  expect(
    (await page.evaluate(() => window.airportControl.snapshot())).renderer
      .surfaceLayers["airport-boundary"],
  ).toBeTruthy();
  for (const layer of [
    "navigation-fixes",
    "procedures",
  ...
  const airspaceSnapshot = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  ...
  const orientationResult = await page.evaluate(() =>
    window.airportControl.request({
      action: "setMapOrientationVisible",
      enabled: false,
    }),
  );
...
test("Manual ORD supports live procedure control, ownership handoffs, and physical separation options", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "The live ATC protocol is covered once in desktop Chromium.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=manual&station=approach&density=quiet&autostart=1&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  const pauseResult = await page.evaluate(() =>
    window.airportControl.request({ action: "pause" }),
  ...
  const arrival = await page.evaluate(() =>
    window.airportControl
      .snapshot()
      .flights.find((flight) => flight.phase === "approach"),
  );
  ...
  await expect(
    page.getByRole("button", { name: `Divert ${arrival!.origin}` }),
  ).toBeVisible();
  const routeEditor = page.locator(".route-editor");
  await expect(routeEditor).toBeVisible();
  await routeEditor.locator("summary").click();
  const routeOptionButtons = routeEditor.locator(
    ".route-editor__options button",
  );
  expect(await routeOptionButtons.count()).toBeGreaterThan(1);
  const routeOptions = await routeOptionButtons.evaluateAll((buttons) =>
    buttons.map(
      (button) =>
        (button as HTMLButtonElement).dataset.routeFixes
          ?.split(",")
          .filter(Boolean) ?? [],
    ),
  );
  ...
  for (const fixIds of routeOptions.slice(1)) {
    ...
    const nextPreview = await page.evaluate(
      ({ flightId, fixIds: nextFixIds }) =>
        window.airportControl.request({
          action: "previewRoute",
          flightId,
          fixIds: nextFixIds,
    ...
    previewState = nextPreview.resultingState.flights.find(
      (flight) => flight.id === arrival!.id,
    )?.navigation.routeClearance;
  }
  expect(previewState).toMatchObject({ status: "preview", safeToIssue: true });
  await expect(
    page.locator('.route-clearance[data-status="preview"]'),
  ).toContainText(/Route preview|SAFE/);
  ...
  const issuedRoute = await page.evaluate(
    (flightId) =>
      window.airportControl.request({
  ...
  expect(acceptedRoute).toMatchObject({
    accepted: true,
    reason: expect.stringContaining("route amendment accepted"),
  });
  expect(
    acceptedRoute.resultingState.flights.find(
      (flight) => flight.id === arrival!.id,
    )?.navigation,
  ).toMatchObject({
    readbackStatus: "accepted",
    routeClearance: { status: "accepted" },
  });
  const resumeResult = await page.evaluate(() =>
    window.airportControl.request({ action: "resume" }),
  );
  ...
  const headingCommand = await page.evaluate((flightId) => {
    const liveFlight = window.airportControl
      .snapshot()
      .flights.find((flight) => flight.id === flightId);
    if (!liveFlight)
      throw new Error(
        `Flight ${flightId} disappeared before the heading command.`,
      );
    const currentHeading =
      (((90 - (liveFlight.motion.heading * 180) / Math.PI) % 360) + 360) % 360;
    const headingDegrees = (currentHeading + 10) % 360;
    return {
      headingDegrees,
      result: window.airportControl.request({
  ...
  expect(
    heading.resultingState.flights.find((flight) => flight.id === arrival!.id)
      ?.navigation.vector,
  ).toBeTruthy();
  await page.waitForFunction(
    ({ flightId, commandId }) =>
  ...
  const causalVector = await page.evaluate(
    ({ flightId, commandId }) =>
      window.airportControl
        .events(30)
        .find(
  ...
  expect(causalVector).toMatchObject({
    protocolVersion: "1.2.0",
    apiVersion: "2.40.0",
    causedByCommandId: heading.commandId,
  });
  ...
  expect(
    (
      await page.evaluate(
        ...
        { flightId: arrival!.id, speedKts: speed },
      )
    ).accepted,
  ).toBeTruthy();
  ...
  expect(
    (
      await page.evaluate(
        ...
        { flightId: arrival!.id, fixId: directFix },
      )
    ).accepted,
  ).toBeTruthy();
  expect(
  ...
  expect(held?.navigation.hold).toMatchObject({
    expectFurtherClearanceAtSeconds: expect.any(Number),
    cycle: 1,
  });
  ...
  expect(
    (
      await page.evaluate(
        (flightId) =>
          window.airportControl.request({ action: "releaseHold", flightId }),
        arrival!.id,
      )
    ).accepted,
  ).toBeTruthy();
  expect(
    (
      await page.evaluate(
        (flightId) =>
          window.airportControl.request({ action: "clearApproach", flightId }),
        arrival!.id,
      )
    ).accepted,
  ).toBeTruthy();
  ...
  const offeredHandoff = await page.evaluate(
    (flightId) =>
      window.airportControl.request({
        action: "offerHandoff",
        flightId,
  ...
  expect(
    offeredHandoff.resultingState.flights.find(
      (flight) => flight.id === arrival!.id,
    )?.navigation,
  ).toMatchObject({
  ...
  await expect(page.locator("#coordination-inbox")).toContainText(
    arrival!.callsign,
  );
  await page
    .locator('#coordination-inbox button[data-coordination-action="accept"]')
    .click();
  await page.waitForFunction(
    (flightId) =>
      window.airportControl
        .snapshot()
        .flights.find((flight) => flight.id === flightId)?.navigation.handoff
        ?.status === "accepted",
    arrival!.id,
  );
  await page.screenshot({
    path: testInfo.outputPath("tower-coordination-inbox.png"),
  });
  expect(
    (
      await page.evaluate(() =>
  ...
  await expect(
    page.locator(
      '#coordination-inbox button[data-coordination-action="contact"]',
    ),
  ).toBeVisible();
  await page
    .locator('#coordination-inbox button[data-coordination-action="contact"]')
    .click();
  await page.waitForFunction(
    (flightId) =>
      window.airportControl
  ...
  const wrongOwner = await page.evaluate(
    ({ flightId, headingDegrees }) =>
      window.airportControl.request({
        action: "assignHeading",
        flightId,
        headingDegrees,
  ...
  expect(wrongOwner).toMatchObject({
    accepted: false,
    reason: expect.stringMatching(/does not own|handoff required/),
  });
  expect(
    (
      await page.evaluate(() =>
  ...
  expect(
    (
      await page.evaluate(
        ...
        { flightId: arrival!.id, runway: arrival!.runway },
      )
    ).accepted,
  ).toBeTruthy();
  expect(
  ...
  expect(controlled.separationRuleset).toMatchObject({
    ruleset: { id: "realistic", radarHorizontalNm: 3, verticalFt: 1_000 },
  });
  expect(
    controlled.flights.find((flight) => flight.id === arrival!.id)?.navigation,
  ).toMatchObject({
    frequencyOwner: "tower",
  ...
  expect(
    controlled.flights
      .find((flight) => flight.id === arrival!.id)
      ?.flightPlan.amendments.some((amendment) =>
        amendment.detail.includes("3,000 ft"),
      ),
  ).toBeTruthy();
  await page.screenshot({ path: testInfo.outputPath("manual-live-atc.png") });
});

test("Group select exposes and applies only shared atomic commands", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Grouped ATC commands are covered once in desktop Chromium.",
  );
  test.setTimeout(120_000);
  await page.goto(
  ...
  await expect(page.locator("#group-select-toggle")).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  for (const flight of arrivals)
    await page.locator(`button[data-flight-chip="${flight.id}"]`).click();
  await expect(page.locator("#group-select-count")).toHaveText("2 selected");
  await expect(page.locator("#group-actions")).toBeVisible();
  await expect(
    page.locator('#group-actions button[data-group-instruction="slow"]'),
  ).toBeVisible();
  await expect(
    page.locator('#group-actions button[data-group-instruction="normal"]'),
  ).toHaveCount(0);
  await expect(
    page.locator('#group-actions button[data-group-instruction="hold"]'),
  ).toHaveCount(0);
  await expect(page.locator("#group-actions")).toContainText(
    "approach authority",
  );
  await page.screenshot({
    path: testInfo.outputPath("grouped-atc-selection.png"),
  });
  ...
  expect(rejected).toMatchObject({
    accepted: false,
    reason: expect.stringContaining("single-flight only"),
  });

  await page
    .locator('#group-actions button[data-group-instruction="slow"]')
    .click();
  await expect(page.locator("#group-select-toggle")).toHaveAttribute(
    "aria-pressed",
    "false",
  ...
  const result = await page.evaluate(
    (flightIds) => {
      ...
      return {
        ...
        groupEvents: window.airportControl
          .events(20)
          .filter((event) => event.type === "group-instruction"),
      };
    },
    arrivals.map((flight) => flight.id),
  );
  ...
  expect(result.controls).toEqual(
    arrivals.map((flight) => ({ id: flight.id, pace: 0.55 })),
  );
  expect(result.groupEvents.map((event) => event.flightId)).toEqual(
    expect.arrayContaining(arrivals.map((flight) => flight.id)),
  );
});

test("ORD snow exposes the deicing route and holdover model in the normal UI", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Winter operations are viewport-independent and covered once in Chromium.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=auto&autostart=1&detail=low&weather=snow&windDir=270&wind=12&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().renderer.context.status === "loaded",
  ...
  await expect(page.locator("#weather-condition-select option")).toHaveCount(7);
  ...
  expect(initial.weather.ceilingFt).toBe(900);
  expect(initial.weather.temperatureC).toBeLessThan(0);
  expect(initial.weather.runwayConditionReports).toHaveLength(
    initial.runways.length,
  );
  expect(initial.weather.runwayConditionReports).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        codes: expect.arrayContaining([expect.any(Number)]),
  ...
  expect(initial.surfaceGraph.deicingFacilities).toHaveLength(1);
  const planned = initial.flights.find(
    (flight) =>
      flight.phase === "resting" && flight.deicing.status === "planned",
  );
  ...
  if (
    await page
      .locator("#flight-strip")
      .evaluate((element) =>
        element.classList.contains("flight-strip--collapsed"),
      )
  ) {
    await page.locator("#flight-strip-toggle").click();
  }
  ...
  await expect(page.locator(".deicing-panel")).toContainText(
    "Central Deicing Facility",
  );
  await expect(page.locator(".deicing-panel__detail")).toContainText(/Lane \d/);
  await expect(
    page
      .locator(".flight-chip__detail")
      .filter({ hasText: "Central Deicing Facility" })
      .first(),
  ).toBeVisible();
  ...
  await page.screenshot({
    path: testInfo.outputPath("ord-winter-deicing.png"),
  });
});

test("Thunderstorm hazards remain opt-in and expose modeled runway reports", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Operational weather state is viewport-independent and covered once in Chromium.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=auto&autostart=1&detail=low&weather=thunderstorm&windDir=240&wind=25&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().renderer.context.status === "loaded",
  ...
  expect(
    initial.weather.runwayConditionReports.every(
      (report) =>
        report.notForNavigation === true &&
        report.source === "modeled-rcam-schematic" &&
        report.codes.length === 3,
    ),
  ).toBeTruthy();

  await page.locator("#menu-toggle").click();
  await expect(page.locator("#weather-condition-select")).toHaveValue(
    "thunderstorm",
  );
  await expect(page.locator("#weather-runway-condition")).toContainText(
    "RwyCC",
  );
  const enabled = await page.evaluate(() =>
    window.airportControl.request({
      action: "setWeatherHazardsEnabled",
      enabled: true,
    }),
  );
  expect(enabled.accepted).toBeTruthy();
  expect(enabled.resultingState.weather.hazardsEnabled).toBeTruthy();
  await expect(page.locator("#high-stakes-weather-enabled")).toBeChecked();
  ...
  expect(disabled.resultingState.weather).toMatchObject({
    hazardsEnabled: false,
    activeHazard: null,
  });
  await expect(page.locator("#high-stakes-weather-enabled")).not.toBeChecked();
});

test("Go-around climbs from the live pose and flies a visible missed-approach path", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "The authoritative go-around is viewport-independent and covered once in Chromium.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ATL&mode=auto&autostart=1&detail=low&speed=3&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(() => {
    const flight = window.airportControl
      .snapshot()
      .flights.find((candidate) => candidate.phase === "approach");
  ...
  const instructed = result.resultingState.flights.find(
    (flight) => flight.id === before.id,
  )!;
  expect(instructed.goAround).toMatchObject({
    detail: "controller instruction",
  ...
  await page.waitForFunction(
    ({ flightId, altitude }) => {
      ...
      return Boolean(
        flight?.goAround &&
        flight.goAround.stage === "go-around-climb" &&
        flight.kinematics.altitudeFt > altitude + 100 &&
        flight.motion.pitch > 0.18,
      );
    },
...
test("No-fail training teaches, explains rejection, and restores a live checkpoint", async ({
  ...
  expect(started.resultingState.schemaVersion).toBe(42);
  ...
  await expect(page.locator("#training-title")).toHaveText(
    "Arrival foundations",
  );
  await expect(page.locator("#training-objective")).toContainText(
    "Select an aircraft",
  );

  const arrival = started.resultingState.flights.find(
    (flight) => flight.phase === "approach",
  );
  expect(arrival).toBeTruthy();
  ...
  expect(
    (
      await page.evaluate(() =>
        window.airportControl.request({ action: "continueTraining" }),
      )
    ).accepted,
  ).toBeTruthy();
  ...
  expect(rejected.resultingState.training.feedback).toContain(
  ...
  await expect(page.locator("#training-explanation")).toHaveAttribute(
    "open",
    "",
  );
  await expect(page.locator("#training-feedback")).toContainText(
    "speed must be",
  );
  await page.screenshot({
  ...
  const retried = await page.evaluate(() =>
    window.airportControl.request({ action: "retryTrainingStep" }),
  );
  ...
  expect(retried.resultingState.training.feedback).toContain(
  ...
  const hint = await page.evaluate(() =>
    window.airportControl.request({ action: "trainingHint" }),
  );
...
test("Challenge shifts lock conditions, grade operations, and fit responsive play", async ({
  page,
}, testInfo) => {
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=auto&challenge=rush-hour&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().renderer.context.status === "loaded",
  ...
  expect(briefing.schemaVersion).toBe(42);
  ...
  expect(
    Object.values(briefing.controllers.automation).every(Boolean),
  ).toBeTruthy();
  ...
  await expect(
    page.locator("#challenge-objectives .challenge-objective"),
  ).toHaveCount(4);
  ...
  await expect(page.locator("#challenge-setup")).not.toHaveAttribute(
    "open",
    "",
  );
  ...
  const started = await page.evaluate(() =>
    window.airportControl.request({ action: "beginChallenge" }),
  );
  expect(started.accepted).toBeTruthy();
  ...
  const live = await page.evaluate(
    () => window.airportControl.snapshot().challenge,
  );
  expect(live.summary.safety.missedHandoffs).toBe(0);
  expect(live.summary.safety.score).toBe(100);
  await page.screenshot({
    path: testInfo.outputPath("challenge-rush-active.png"),
  });
  ...
  if (testInfo.project.name === "mobile-chromium") {
    expect(
      await page
        .locator("#challenge-end")
        .evaluate((element) => element.getBoundingClientRect().height),
    ).toBeGreaterThanOrEqual(44);
  } else {
    await page.setViewportSize({ width: 1024, height: 600 });
    viewport = page.viewportSize()!;
    const laptopHud = await page
    ...
    const overlapsFlightStrip =
      laptopHud.left < laptopStrip.right &&
    ...
    await page.screenshot({
      path: testInfo.outputPath("challenge-rush-laptop.png"),
    });
  }

  const ended = await page.evaluate(() =>
    window.airportControl.request({ action: "endChallenge" }),
  );
  expect(ended.accepted).toBeTruthy();
  expect(ended.resultingState.challenge).toMatchObject({
    status: "abandoned",
    grade: "F",
  });
  await expect(page.locator("#challenge-results")).toBeVisible();
  await expect(page.locator("#challenge-result-grade")).toHaveText("F");
  await expect(page.locator(".challenge-results__summary > span")).toHaveCount(
    6,
  );
  await expect(
    page.locator("#challenge-result-objectives .challenge-objective"),
  ).toHaveCount(4);
  await expect(page.locator("#challenge-retry")).toBeFocused();
  const resultBounds = await page
    .locator(".challenge-results__panel")
    .evaluate((element) => {
      const rect = element.getBoundingClientRect();
      return {
        top: rect.top,
  ...
  await page.screenshot({
    path: testInfo.outputPath("challenge-rush-debrief.png"),
  });
  ...
  await expect(page.locator("#challenge-hud")).toBeVisible();
  expect(
    (await page.evaluate(() => window.airportControl.snapshot())).challenge
      .status,
  ).toBe("briefing");
  expect(
    (
...
test("Sandbox stages requested traffic without score pressure and fits responsive play", async ({
  page,
}, testInfo) => {
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=auto&sandbox=1&autostart=1&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().renderer.context.status === "loaded",
  ...
  await expect(page.locator("#sandbox-hud")).toContainText(
    "No score · no fail",
  );
  ...
  await page.waitForFunction(
    () =>
      window.airportControl.snapshot().sandbox.totals.releasedArrivals === 1,
  );
  ...
  const configured = await page.evaluate(() => {
    ...
    const manual = window.airportControl.request({
      action: "setMode",
      value: "manual",
    });
    ...
    const cancelled = window.airportControl.request({
      action: "cancelSandboxInjections",
    });
    return {
      weather,
      manual,
  ...
  expect(configured.snapshot.weather).toMatchObject({
    condition: "rain",
    windSpeed: 18,
  });
  ...
  if (testInfo.project.name === "mobile-chromium") {
    expect(
      await page
        .locator("#sandbox-inject")
        .evaluate((element) => element.getBoundingClientRect().height),
    ).toBeGreaterThanOrEqual(44);
  }
  await page.screenshot({
    path: testInfo.outputPath(`sandbox-${testInfo.project.name}.png`),
  });

  const cleared = await page.evaluate(() =>
    window.airportControl.request({ action: "clearSandboxTraffic" }),
  );
  expect(cleared.accepted).toBeTruthy();
  expect(cleared.resultingState.sandbox.activeAircraftCount).toBe(0);
  expect(cleared.resultingState.weather).toMatchObject({
    condition: "rain",
    windSpeed: 18,
  });
  const stopped = await page.evaluate(() =>
    window.airportControl.request({ action: "stopSandbox" }),
  );
...
test("Unified input keeps held keys smooth and supports a standard gamepad without bypassing UI context", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers keyboard and the optional standard-gamepad adapter.",
  );
  test.setTimeout(120_000);
  ...
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().input.devices.gamepad.connected,
  );
  ...
  const driftFocusId = initial.flights[0]?.id;
  expect(driftFocusId).toBeDefined();
  const driftFocus = await page.evaluate(
    (flightId) =>
      window.airportControl.request({ action: "focusFlight", flightId }),
    driftFocusId!,
  );
  expect(driftFocus.accepted).toBeTruthy();
  await page.evaluate(() => {
    const pad = (
      window as unknown as {
        __airportTestGamepad: { axis(index: number, value: number): void };
      }
    ).__airportTestGamepad;
    pad.axis(0, 0.22);
  });
  ...
  expect(afterDrift.selection.focusedFlightId).toBe(driftFocusId);
  expect(afterDrift.renderer.camera.target?.key).toBe(`flight:${driftFocusId}`);
  expect(afterDrift.input.axes.panX).toBe(0);
  await page.evaluate(() => {
    (
      window as unknown as {
        __airportTestGamepad: { axis(index: number, value: number): void };
      }
    ).__airportTestGamepad.axis(0, 0);
  });
  ...
  const keyMid = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.waitForTimeout(220);
  const keyEnd = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.keyboard.up("d");
  expect(
    Math.hypot(
      keyMid.focusX - keyStart.focusX,
      keyMid.focusY - keyStart.focusY,
    ),
  ).toBeGreaterThan(1);
  expect(
    Math.hypot(keyEnd.focusX - keyMid.focusX, keyEnd.focusY - keyMid.focusY),
  ).toBeGreaterThan(1);
  await page.waitForTimeout(80);
  const keyReleased = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.waitForTimeout(180);
  const keyStopped = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  expect(
    Math.hypot(
      keyStopped.renderer.camera.focusX - keyReleased.focusX,
      keyStopped.renderer.camera.focusY - keyReleased.focusY,
    ),
  ).toBeLessThan(1);
  ...
  expect(
    await page.evaluate(() => window.airportControl.snapshot().input.context),
  ).toBe("ui");
  // The airport selector deliberately receives first focus for accessibility.
  // Probe gameplay blocking from a non-editable drawer control so native
  // select typeahead cannot change airports during this input assertion.
  await page.locator(".advanced-tools > summary").focus();
  const drawerCamera = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.keyboard.press("d");
  await page.waitForTimeout(200);
  const blockedCamera = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  ...
  await page.screenshot({
    path: testInfo.outputPath("unified-input-settings.png"),
  });
  await page.keyboard.press("Escape");
  await expect(page.locator("#control-panel")).not.toHaveClass(
    /control-panel--open/,
  );

  const invalidSensitivity = await page.evaluate(() =>
    window.airportControl.request({
      action: "setGamepadSensitivity",
      sensitivity: 3,
    }),
  );
  ...
  expect(sensitivity.resultingState.input.devices.gamepad.sensitivity).toBe(
    1.4,
  );

  const gamepadStart = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera,
  );
  await page.evaluate(() => {
    const pad = (
  ...
  expect(
    await page.evaluate(
      () => window.airportControl.snapshot().input.lastDevice,
    ),
  ).toBe("gamepad");
  ...
  await page.evaluate(() => {
    (
      window as unknown as {
        __airportTestGamepad: { button(index: number, pressed: boolean): void };
      }
    ).__airportTestGamepad.button(5, true);
  });
  await page.waitForFunction(
    () => window.airportControl.snapshot().selection.focusedFlightId !== null,
  );
  expect(
    await page.evaluate(
      () => window.airportControl.snapshot().input.lastAction,
    ),
  ).toBe("selection.next");
  await page.evaluate(() => {
    const pad = (
  ...
  await page.evaluate(() => {
    ...
    ).__airportTestGamepad.button(1, false);
  });
  ...
  const persisted = await page.evaluate(
    () => window.airportControl.snapshot().input.devices.gamepad,
  );
  expect(persisted.enabled).toBeFalsy();
  expect(persisted.sensitivity).toBe(1.4);
});

test("Environment controls, accessible palettes, and the camera director remain presentation-only", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers automatic scene presentation and manual camera yield.",
  );
  test.setTimeout(90_000);
  await page.goto(
    "/?airport=ORD&mode=watch&autostart=1&detail=low&renderFps=2&lighting=automatic&season=automatic",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  const initial = await page.evaluate(() => window.airportControl.snapshot());
  expect(initial.schemaVersion).toBe(42);
  ...
  expect(initial.presentation).toMatchObject({
    ...
    cameraDirector: { schemaVersion: 1, enabled: false, status: "off" },
  });

  await page.locator("#menu-toggle").click();
  await page.locator(".advanced-tools > summary").click();
  await expect(page.locator("#lighting-mode-select")).toBeVisible();
  await page.selectOption("#lighting-mode-select", "night");
  await page.waitForFunction(
    () => window.airportControl.snapshot().environment.lightingMode === "night",
  );
  await expect(page.locator("body")).toHaveClass(/night-mode/);
  await expect(page.locator("#environment-readout")).toContainText(
    "forced night",
  );
  await page.selectOption("#season-mode-select", "winter");
  await page.selectOption("#accessibility-palette-select", "cvd-safe");
  await expect(page.locator("html")).toHaveAttribute(
    "data-accessibility-palette",
    "cvd-safe",
  );
  const selected = await page.evaluate(() => window.airportControl.snapshot());
  ...
  expect(selected.presentation.accessibilityPalette).toBe("cvd-safe");
  expect(selected.renderer.accessibilityPalette).toBe("cvd-safe");
  ...
  expect(dayResult.accepted).toBeTruthy();
  expect(dayResult.resultingState.environment).toMatchObject({
    lightingMode: "day",
    phase: "day",
    daylight: 1,
  });
  ...
  expect(paletteResult.accepted).toBeTruthy();

  await page.locator("#menu-toggle").click();
  const directorResult = await page.evaluate(() =>
    window.airportControl.request({
      action: "setCameraDirectorEnabled",
      enabled: true,
    }),
  );
  expect(directorResult.accepted).toBeTruthy();
  await page.waitForFunction(() => {
    const snapshot = window.airportControl.snapshot();
    return (
      snapshot.presentation.cameraDirector.status === "following" &&
      snapshot.selection.focusedFlightId !== null
    );
  });
  const directed = await page.evaluate(() => window.airportControl.snapshot());
  expect(directed.presentation.cameraDirector.targetFlightId).toBe(
    directed.selection.focusedFlightId,
  );
  ...
  await page.waitForFunction(() => {
    const snapshot = window.airportControl.snapshot();
    return (
      !snapshot.presentation.cameraDirector.enabled &&
      snapshot.selection.focusedFlightId === null
    );
  });
  const yielded = await page.evaluate(() => window.airportControl.snapshot());
  expect(yielded.presentation.cameraDirector).toMatchObject({
    enabled: false,
    status: "yielded",
  });
  expect(yielded.presentation.cameraDirector.reason).toContain("Manual camera");

  await page.reload();
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  const persisted = await page.evaluate(() => window.airportControl.snapshot());
  expect(persisted.presentation.accessibilityPalette).toBe("high-contrast");
  await expect(page.locator("html")).toHaveAttribute(
    "data-accessibility-palette",
    "high-contrast",
  );
...
test("Operations data lab records authoritative traces and exports local analysis", async ({
  ...
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers the local analytics recorder and responsive lab.",
  );
  test.setTimeout(90_000);
  await page.setViewportSize({ width: 1024, height: 600 });
  await page.goto("/?airport=ORD&mode=auto&autostart=1&detail=low&renderFps=4");
  ...
  const api = await page.evaluate(() => {
    const overview = window.airportControl.snapshot().analytics;
    ...
    return {
      ...
      json: {
        ...
        flightRecorderSamples: json.flightRecorder.length,
        localOnly: json.disclosure.localOnly,
  ...
  const lab = page.locator("#operations-lab");
  ...
  expect(
    await lab.locator("[data-operations-altitude]").getAttribute("d"),
  ).toMatch(/^M/);
  expect(await lab.locator("[data-operations-heatmap] line").count()).toBe(8);

  const downloadPromise = page.waitForEvent("download");
  await lab.locator("[data-operations-dataset]").selectOption("shift-metrics");
  await lab.locator("[data-operations-export-csv]").click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe("ord-operations-shift-metrics.csv");
  ...
  expect(desktopBounds.top).toBeGreaterThanOrEqual(0);
  expect(desktopBounds.left).toBeGreaterThanOrEqual(0);
  expect(desktopBounds.right).toBeLessThanOrEqual(1024);
  expect(desktopBounds.bottom).toBeLessThanOrEqual(600);

  await page.setViewportSize({ width: 390, height: 844 });
  const mobileBounds = await lab.evaluate((element) => {
    const rect = element.getBoundingClientRect();
    const scroll = element.querySelector(".operations-lab__scroll");
    return {
      top: rect.top,
      right: rect.right,
      bottom: rect.bottom,
  ...
  expect(mobileBounds.bottom).toBeLessThanOrEqual(844);
  expect(mobileBounds.scrollHeight).toBeGreaterThan(mobileBounds.clientHeight);
  await page.screenshot({
    path: testInfo.outputPath("mobile-operations-data-lab.png"),
  });

  await lab.locator("[data-operations-close]").click();
  await expect(lab).toBeHidden();
});

test("Replay inspector verifies, migrates, compares, imports, and shares exact seeds", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers the portable replay workflow and responsive inspector.",
  );
  test.setTimeout(90_000);
  await page.setViewportSize({ width: 1024, height: 700 });
  await page.goto(
    "/?airport=LOCAL&seed=4242&mode=auto&autostart=1&detail=low&renderFps=4",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
  ...
  const api = await page.evaluate(async () => {
    const recording = window.airportControl.recording();
    const exact = window.airportControl.replayTools.verify(recording);
    const asyncExact =
      await window.airportControl.replayTools.verifyAsync(recording);
    const tampered = structuredClone(recording);
    tampered.frames[1].state.elapsed += 9;
    const rejected = window.airportControl.replayTools.load(tampered);
    const legacy = structuredClone(recording) as unknown as Record<
      string,
      unknown
    >;
    ...
    const migrated = window.airportControl.replayTools.verify(legacy);
    const shareable = window.airportControl.replayTools.shareable(recording);
    const comparison = window.airportControl.replayTools.compare(
      0,
    ...
    return {
      recording: {
        ...
        frameHashes: recording.integrity.frameHashes.length,
        manifestHash: recording.integrity.manifestHash,
        sessionId: recording.sessionId,
      },
      exact: {
        accepted: exact.accepted,
        exact: exact.exact,
        checkedFrames: exact.checkedFrames,
        manifestHash: exact.manifestHash,
      },
      asyncExact: asyncExact.exact,
      rejected: {
      ...
      migrated: {
        accepted: migrated.accepted,
        migrated: migrated.migrated,
        legacyUnsealed: migrated.legacyUnsealed,
        schemaVersion: migrated.schemaVersion,
      },
      shareable: {
        disclosure: shareable.sharing,
        sessionId: shareable.sessionId,
        exact: window.airportControl.replayTools.verify(shareable).exact,
        serialized: JSON.stringify(shareable),
      },
      comparison,
      seedLink: window.airportControl.replayTools.seedLink(),
    };
  });
  expect(api.recording).toMatchObject({
    ...
    manifestHash: expect.stringMatching(/^[a-f0-9]{16}$/),
  });
  expect(api.recording.frameHashes).toBe(api.recording.frames);
  expect(api.recording.markers).toBeGreaterThan(0);
  expect(api.exact).toMatchObject({
    accepted: true,
    exact: true,
    checkedFrames: api.recording.frames,
    manifestHash: api.recording.manifestHash,
  });
  expect(api.asyncExact).toBeTruthy();
  expect(api.rejected.accepted).toBeFalsy();
  expect(api.rejected.exact).toBeFalsy();
  expect(api.rejected.mismatches).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ scope: "frame", index: 1 }),
    ]),
  );
  expect(api.migrated).toMatchObject({
    accepted: true,
    migrated: true,
    legacyUnsealed: true,
    schemaVersion: 4,
  });
  expect(api.shareable.disclosure).toMatchObject({
  ...
  expect(api.shareable.sessionId).toBe("shared-session");
  expect(api.shareable.exact).toBeTruthy();
  expect(api.shareable.serialized).not.toContain(api.recording.sessionId);
  expect(api.comparison?.equal).toBeFalsy();
  expect(api.comparison?.differenceCount).toBeGreaterThan(0);
  const seedUrl = new URL(api.seedLink);
  expect(seedUrl.searchParams.get("airport")).toBe("LOCAL");
  expect(seedUrl.searchParams.get("seed")).toBe("4242");
  expect(seedUrl.searchParams.get("autostart")).toBe("1");

  const portableReplay = await page.evaluate(() =>
    JSON.stringify(window.airportControl.recording()),
  );
  await page.locator("#menu-toggle").click();
  await page.locator(".advanced-tools > summary").click();
  const inspector = page.locator("#replay-inspector");
  await expect(inspector).toBeVisible();
  await expect(inspector.locator("#replay-share")).toBeVisible();
  await inspector.locator("#replay-verify").click();
  await expect(inspector.locator("#replay-verification")).toContainText(
    "Exact fingerprints verified",
  );

  await page.locator("#replay-toggle").click();
  await inspector.locator("#replay-baseline").click();
  await page.locator("#replay-slider").fill("0");
  await inspector.locator("#replay-compare").click();
  await expect(inspector.locator("#replay-comparison")).toContainText(
    /state change/,
  );
  await expect(inspector.locator("#replay-marker option")).not.toHaveCount(1);

  await inspector.locator("#replay-file").setInputFiles({
    name: "local-4242.airport-auto-replay.json",
    mimeType: "application/json",
    buffer: Buffer.from(portableReplay),
  });
  await expect(inspector.locator("#replay-source")).toContainText(
    "imported replay",
  );
  await expect(inspector.locator("#replay-verification")).toContainText(
    "Exact fingerprints verified",
  ...
  expect(
    (await page.evaluate(() => window.airportControl.snapshot())).replay,
  ).toMatchObject({
    schemaVersion: 4,
    source: "imported",
  ...
  const desktopBounds = await inspector.evaluate((element) => {
    const rect = element.getBoundingClientRect();
    return {
      top: rect.top,
      right: rect.right,
  ...
  await page.screenshot({
    path: testInfo.outputPath("mobile-replay-inspector.png"),
  });

  await inspector.locator("#replay-live-buffer").click();
  await expect(inspector.locator("#replay-source")).toHaveText("Live buffer");
});

test("Controller policies expose capacity, preserve safety authority, and survive airport changes", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers the controller-policy controls and protocol.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ORD&mode=watch&station=supervisor&autostart=1&detail=low&renderFps=1",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () =>
      window.airportControl.snapshot().controllers.scripted.decisions.length >
      0,
  );
  ...
  expect(
    Object.values(initial.controllers.scripted.stations).every(
      (station) =>
  ...
  await page.waitForFunction(
    () =>
      window.airportControl.snapshot().controllers.policy.selected === "calm",
  );
  await expect(page.locator("#controller-policy-detail")).toContainText("ASMR");
  await expect(
    page.locator('[data-station-workload="approach"] span'),
  ).toContainText("/");

  await page.evaluate(() =>
    window.airportControl.request({ action: "setStation", station: "tower" }),
  );
  await expect(page.locator("#controller-policy-select")).toBeDisabled();
  const authorityRejected = await page.evaluate(() =>
    window.airportControl.dispatch({
      protocolVersion: "1.2.0",
  ...
  expect(authorityRejected).toMatchObject({
    accepted: false,
  ...
  expect(policyChanged.resultingState.controllers.policy.selected).toBe(
    "efficient",
  );
  expect(
    policyChanged.resultingState.controllers.scripted.stations.ground.policy
      .maxActionsPerEvaluation,
  ).toBe(3);

  const changedAirport = await page.evaluate(() =>
    window.airportControl.request({ action: "selectAirport", code: "JFK" }),
  );
  expect(changedAirport.accepted).toBeTruthy();
  expect(changedAirport.resultingState).toMatchObject({
    airport: { code: "JFK" },
    controllers: {
      policy: { selected: "efficient" },
      scripted: { presetId: "efficient" },
    },
  });
  expect(
    changedAirport.resultingState.controllers.scripted.transitions.length,
  ).toBeLessThanOrEqual(32);
});

test("Observer focus follows live traffic, airport assets, queues, and conflicts without stealing camera control", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop Chromium covers the complete focus catalog and moving follow behavior.",
  );
  test.setTimeout(180_000);
  await page.goto(
    "/?airport=ORD&mode=watch&density=busy&autostart=1&detail=low&renderFps=4",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () =>
  ...
  await page.waitForFunction(() =>
    window.airportControl
      .snapshot()
      .focus.catalog.categories.every(
        (category) => category.kind === "conflict" || category.count > 0,
      ),
  );
  ...
  expect(
    Object.values(initial.controllers.scripted.stations).every(
      (station) => station.mode === "scripted" && station.evaluations > 0,
    ),
  ).toBeTruthy();
  expect(
    initial.controllers.scripted.decisions.some(
      (decision) => decision.accepted && decision.producedEventTypes.length > 0,
    ),
  ).toBeTruthy();
  const scriptedEvent = await page.evaluate(() =>
    window.airportControl
      .events(100)
      .find((event) => event.type === "controller-decision"),
  );
  ...
  expect(initial.focus).toMatchObject({
    schemaVersion: 1,
    current: null,
    catalog: { schemaVersion: 1 },
  });
  expect(
    initial.focus.catalog.categories.map((category) => category.kind),
  ).toEqual(["flight", "runway", "taxiway", "gate", "queue", "conflict"]);
  expect(
    initial.focus.catalog.targets.filter((target) => target.kind === "runway"),
  ).toHaveLength(8);
  expect(
    initial.focus.catalog.targets.filter((target) => target.kind === "taxiway")
      .length,
  ).toBeGreaterThanOrEqual(100);
  expect(
    initial.focus.catalog.targets.filter((target) => target.kind === "gate"),
  ).toHaveLength(40);
  expect(initial.input.actions).toContainEqual(
    expect.objectContaining({ id: "ui.focus", keyboardCodes: ["KeyF"] }),
  );
  ...
  await page.waitForFunction(
    (key) => window.airportControl.snapshot().focus.current?.key === key,
    runwayKey,
  );
  await page.waitForFunction(
    (key) =>
      window.airportControl.snapshot().renderer.camera.target?.key === key,
    runwayKey,
  );
  const runwayFocused = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  expect(runwayFocused.focus.current).toMatchObject({
    kind: "runway",
    follow: "static",
    flightIds: [],
  });
  expect(runwayFocused.renderer.camera.target).toMatchObject({
    kind: "runway",
    tracking: true,
  });
  ...
  const directTargets = await page.evaluate(() => {
    ...
    const movingFlight = snapshot.flights.find(
      (flight) =>
        flight.phase === "approach" ||
    ...
    return {
      ...
      flight: catalog.find(
        (target) =>
          target.kind === "flight" && target.id === String(movingFlight?.id),
      ),
    };
  });
  ...
  for (const target of [
    ...
    expect(response.resultingState.renderer.camera.target?.key).toBe(
      target!.key,
    );
  }
  if (directTargets.conflict) {
    ...
    expect(
      conflict.resultingState.focus.current?.flightIds.length,
    ).toBeGreaterThanOrEqual(2);
  }
  ...
  await page.waitForFunction(
    () => window.airportControl.snapshot().focus.current?.kind === "queue",
  );
  expect(
    await page.evaluate(
      () => window.airportControl.snapshot().renderer.camera.target?.kind,
    ),
  ).toBe("queue");
  await page.evaluate(() =>
    window.airportControl.request({
  ...
  const flightResponse = await page.evaluate(
    (target) =>
      window.airportControl.request({
        action: "focusTarget",
        target: { kind: target!.kind, id: target!.id },
  ...
  const movingFlightFocus = await page.evaluate(() =>
    window.airportControl.snapshot(),
  );
  expect(movingFlightFocus.focus.current?.kind).toBe("flight");
  expect(movingFlightFocus.selection.focusedFlightId).toBe(
    Number(directTargets.flight!.id),
  );
  expect(movingFlightFocus.renderer.camera.target?.key).toBe(
    directTargets.flight!.key,
  );
  expect(
    Math.hypot(
      movingFlightFocus.renderer.camera.target.resolvedX -
        firstFlightFocus.resolvedX,
      movingFlightFocus.renderer.camera.target.resolvedY -
        firstFlightFocus.resolvedY,
    ),
  ...
  await expect(page.locator("#focus-panel")).toBeVisible();
  await page.screenshot({
    path: testInfo.outputPath("observer-focus-desktop.png"),
  });
  ...
  expect(
    await page.evaluate(
      () => window.airportControl.snapshot().focus.current?.kind,
    ),
  ).toBe("flight");
  await page.locator("#scene").focus();
  ...
  await page.waitForFunction(
    () => window.airportControl.snapshot().focus.current === null,
  );
...
test("Mobile Watch mode keeps non-ORD and procedural maps navigable in low detail", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "mobile-chromium",
    "This test is the dedicated responsive/mobile browser gate.",
  );
  test.setTimeout(120_000);
  await page.goto(
    "/?airport=ATL&mode=watch&autostart=1&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  ...
  const beforePinch = await page.evaluate(
    () => window.airportControl.snapshot().renderer.camera.zoom,
  );
  await page.locator("#scene").evaluate((canvas) => {
    const rect = canvas.getBoundingClientRect();
  ...
  await page.waitForFunction(
    (zoom) =>
      Math.abs(window.airportControl.snapshot().renderer.camera.zoom - zoom) >
      0.02,
    beforePinch,
  );
  expect(
    await page.evaluate(
      () => window.airportControl.snapshot().input.lastGesture,
    ),
  ).toBe("pinch");
  await page.locator("#menu-toggle").click();
  await page
  ...
  expect(
    await page
      .locator(".input-settings__toggle")
      .evaluate((element) => element.getBoundingClientRect().height),
  ).toBeGreaterThanOrEqual(44);
  await page.screenshot({
    path: testInfo.outputPath("mobile-input-settings.png"),
  });
  await page.keyboard.press("Escape");
  await expect(page.locator("#control-panel")).not.toHaveClass(
    /control-panel--open/,
  );
  const safetyFont = await page
    .locator(".scoreboard span")
    .nth(2)
  ...
  const compactFocusControls = await page
    .locator("#focus-panel button, #focus-panel select")
  ...
  await page.locator("#focus-apply").click();
  await page.waitForFunction(
    () => window.airportControl.snapshot().focus.current?.kind === "gate",
  );
  await page.screenshot({
    path: testInfo.outputPath("mobile-observer-focus.png"),
  });
  await page.keyboard.press("Escape");
  await expect(page.locator("#focus-panel")).toBeHidden();
  await expect(page.locator("#focus-status")).toBeVisible();
  ...
  await page.evaluate(() =>
    window.airportControl.request({ action: "selectAirport", code: "LOCAL" }),
  );
  await page.waitForFunction(
    () => window.airportControl.snapshot().airport.code === "LOCAL",
  );
  ...
  expect(
    (
      await page.evaluate(() =>
        window.airportControl.request({ action: "setMode", value: "manual" }),
      )
    ).accepted,
  ).toBeTruthy();
  expect(
  ...
  const mobileBriefing = await page
    .locator("#station-briefing")
    .evaluate((element) => {
      const rect = element.getBoundingClientRect();
      return {
        top: rect.top,
  ...
  expect(mobileBriefing.right).toBeLessThanOrEqual(
    mobileBriefing.viewportWidth,
  );
  expect(mobileBriefing.bottom).toBeLessThanOrEqual(
    mobileBriefing.viewportHeight,
  );
  expect(
    await page
      .locator(".station-briefing__role summary")
      .evaluate((element) => element.getBoundingClientRect().height),
  ).toBeGreaterThanOrEqual(44);
  await page.screenshot({
    path: testInfo.outputPath("mobile-controller-briefing.png"),
  });
});

test("Laptop viewports keep the complete controls menu reachable", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Laptop viewport coverage runs once in desktop Chromium.",
  );
  test.setTimeout(180_000);
  await page.setViewportSize({ width: 1024, height: 600 });
  ...
  await introPanel.evaluate((element) => {
    element.scrollTop = element.scrollHeight;
  });
  await expect(page.locator("#enter")).toBeVisible();

  await page.goto(
    "/?airport=ORD&mode=auto&autostart=1&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.snapshot().renderer.drawCalls > 100,
  );
  const renderBudget = await page.evaluate(
    () => window.airportControl.snapshot().renderer,
  );
  ...
  for (const viewport of [
    ...
    if (
      !(await panel.evaluate((element) =>
        element.classList.contains("control-panel--open"),
      ))
    )
      await menu.click();
    ...
    if (viewport.width >= 640) {
      const controlsBounds = await page
        .locator("#control-panel > .controls")
        .evaluate((element) => {
          const rect = element.getBoundingClientRect();
          return {
            top: rect.top,
    ...
    const advancedBounds = await page
      .locator(".advanced-tools summary")
      .evaluate((element) => {
        const rect = element.getBoundingClientRect();
    ...
    if (viewport.width === 700) {
      ...
      await page.screenshot({
        path: testInfo.outputPath("laptop-zoomed-700x500.png"),
      });
    }
    await menu.click();
  ...
  const compactOverlayBounds = await page
    .locator("#radar-panel, #queue-panel")
    .evaluateAll((elements) =>
  ...
  const overlaysIntersect =
    radarBounds.left < queueBounds.right &&
  ...
  await page.waitForTimeout(200);
  await page.screenshot({
    path: testInfo.outputPath("laptop-controls-1024x600.png"),
  });
  await page.locator("#menu-toggle").click();
  expect(
    (
  ...
  for (const viewport of [
    ...
    await page.setViewportSize(viewport);
    const briefingBounds = await page
      .locator("#station-briefing")
      .evaluate((element) => {
        const rect = element.getBoundingClientRect();
        return {
  ...
  await page.waitForTimeout(4_100);
  await page.screenshot({
    path: testInfo.outputPath("laptop-ground-briefing-700x500.png"),
  });
});

test("Runtime budgets stay bounded, visible, and available to local control", async ({
  page,
}) => {
  await page.goto(
    "/?airport=ORD&mode=auto&density=extreme&autostart=1&detail=low&renderFps=0.25",
  );
  await page.waitForFunction(() => window.airportControl?.version === "2.40.0");
  await page.waitForFunction(
    () => window.airportControl.performance().frameWorkMs.samples >= 30,
  );
  await page.waitForFunction(
    () => window.airportControl.performance().retention.counterSamples >= 1,
  );

  const runtime = await page.evaluate(() =>
    window.airportControl.performance(),
  );
  expect(runtime.schemaVersion).toBe(1);
  expect(["warming", "nominal", "attention", "exceeded"]).toContain(
    runtime.status,
  );
  expect(runtime.frameWorkMs.samples).toBeLessThanOrEqual(
    runtime.retention.maximumFrameSamples,
  );
  expect(runtime.simulationTickMs.samples).toBeLessThanOrEqual(
    runtime.retention.maximumSimulationSamples,
  );
  expect(runtime.retention.counterSamples).toBeLessThanOrEqual(
    runtime.retention.maximumCounterSamples,
  );
  expect(runtime.maximumTicksPerFrame).toBeLessThanOrEqual(3);
  expect(runtime.checks.map((check) => check.id)).toEqual(
    expect.arrayContaining([
      "frame-work-p95",
      "frame-gap-p95",
      "simulation-tick-p95",
  ...
  expect(runtime.disclosure).toContain("never relax simulation safety rules");
  const snapshotRuntime = await page.evaluate(
    () => window.airportControl.snapshot().performance,
  );
  expect(snapshotRuntime.schemaVersion).toBe(runtime.schemaVersion);

  await page.locator("#menu-toggle").click();
  await page.locator("#performance-toggle").scrollIntoViewIfNeeded();
  await page.locator("#performance-toggle").click();
  await expect(page.locator("#debug-panel")).toBeVisible();
  await expect(page.locator("#debug-panel")).toContainText("sim p95");
  await expect(page.locator("#performance-toggle")).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  const bounds = await page.locator("#debug-panel").evaluate((element) => {
    const rect = element.getBoundingClientRect();
    return {
      top: rect.top,
      right: rect.right,
