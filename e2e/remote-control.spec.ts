...
class GatewayPeer {
  ...
  constructor(readonly socket: WebSocket) {
    socket.on("message", (data) => {
      const message = JSON.parse(data.toString("utf8")) as Record<
        string,
        unknown
      >;
...
test("opt-in browser host and external controller share the formal safety arbiter", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "The remote transport is viewport-independent.",
  );
  ...
  try {
    ...
    const connected = await page.evaluate(
      ({ endpoint, session, token }) =>
        window.airportControl.remote.connect({
          endpoint,
          sessionId: session,
          token,
        }),
    ...
    await expect(page.locator("#remote-host-state")).toHaveText(
      "Gateway connected",
    );
    ...
    const welcome = await controller.waitFor(
    ...
    const result = await controller.waitFor(
      (message) =>
        ...
        message.requestId === "playwright-remote-pause",
    );
    ...
    await expect
      .poll(() => page.evaluate(() => window.airportControl.snapshot().paused))
      .toBe(true);
    ...
    const disconnected = await page.evaluate(() =>
      window.airportControl.remote.disconnect(),
    );
    ...
    await page
      .evaluate(() => window.airportControl?.remote.disconnect())
      .catch(() => undefined);
    await gateway.close();
  }
});
