...
test("Manual Supervisor can ignore a flow advisory, see consequences, and recover", async ({
  ...
  await page.waitForFunction(
    () =>
      window.airportControl?.snapshot().trafficManagement.recommendations
        .length > 0,
  );
  ...
  const ignore = page
    .locator('#queue-capacity button[data-flow-advisory-action="ignore"]')
    .first();
  ...
  await expect(
    ...
    /Ignored \d+s · \+\d+s queue delay · \+[\d.]+ kg holding fuel/,
  );
  expect(
  ...
  const recover = page
    .locator('#queue-capacity button[data-flow-advisory-action="recover"]')
    .first();
