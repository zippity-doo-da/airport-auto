...
const uncertaintyFixture = new AirportSimulation(ordConfig, 'rush');
const uncertaintyState = structuredClone(uncertaintyFixture.state);
const uncertaintyFlights = uncertaintyState.flights.slice(0, 2);
assert(uncertaintyFlights.length === 2, 'operational uncertainty fixture needs two flights');
uncertaintyFlights[0].phase = 'approach';
uncertaintyFlights[1].phase = 'taxi-out';
...
assert(uncertainForecast.capacityWindows.every((window) => window.predictedDemandCount >= window.demandCount && window.predictedCapacityCount >= window.plannedReleaseCount && window.confidence === 'low' && window.uncertainty.weather === 0.8 && window.uncertainty.procedure > 0.5 && window.uncertainty.taxiCongestion > 0.5 && window.uncertainty.gateReadiness > 0.4 && window.uncertainty.downstreamSaturation > 0.5 && window.attribution.schemaVersion === 2 && window.attribution.capacityProfile.schemaVersion === 1 && window.attribution.capacityProfile.nonNavigational && window.attribution.capacityProfile.id === 'ord-flow-capacity-v1' && window.attribution.configurationId && window.attribution.runways.length > 0 && /procedure|taxi congestion|gate readiness|downstream saturation/.test(window.confidenceReason)), 'rolling forecast did not expose bounded directional operational uncertainty and capacity attribution');
const closedArrivalForecast = trafficFlowSnapshot(flow, 20, { arrivalSpacingSeconds: 24, arrivalAttribution: { ...capacityAttribution.arrival, capacityProfile: { ...capacityAttribution.arrival.capacityProfile, activeIndependentRunwayCount: 0 } } }).capacityWindows.find((window) => window.direction === 'arrival');
assert(closedArrivalForecast && closedArrivalForecast.predictedCapacityCount === closedArrivalForecast.plannedReleaseCount, 'a fully closed modeled runway system projected uncommitted capacity');
