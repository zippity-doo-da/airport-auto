...
import { runwayClosedByDisruption, surfaceDisruptionBlockedEdgeIds } from './src/simulation/surfaceDisruptions.ts';
...
const runwayRecovery = simulation.state.surfaceDisruptions.find((disruption) => disruption.kind === 'disabled-aircraft' && disruption.flightId === flight.id);
assert(runwayRecovery, 'stopped RTO did not create an authoritative recovery disruption');
...
assert(runwayEdges.every((edgeId) => blockedRunwayEdges.has(edgeId)), 'RTO runway edges remained available to surface routing');
...
let recoveryTicks = 0;
for (; recoveryTicks < 4_000 && simulation.state.flights.some((candidate) => candidate.id === flight.id); recoveryTicks += 1) simulation.update(0.05);
assert(!simulation.state.flights.some((candidate) => candidate.id === flight.id), 'RTO aircraft was not towed clear after recovery');
assert(!simulation.state.surfaceDisruptions.some((disruption) => disruption.id === runwayRecovery.id), 'RTO disruption remained after recovery completed');
assert(!runwayClosedByDisruption(simulation.state.surfaceDisruptions, flight.runway), 'RTO runway did not reopen after tow and inspection');
assert(runwayEdges.every((edgeId) => !surfaceDisruptionBlockedEdgeIds(simulation.state.surfaceDisruptions).has(edgeId)), 'RTO runway graph remained blocked after recovery');
...
assert(events.some((event) => event.type === 'recovery-start'), 'RTO recovery-start event was not emitted');
assert(events.some((event) => event.type === 'recovery-complete'), 'RTO recovery-complete event was not emitted');
...
console.log(JSON.stringify({
  ...
  recoverySeconds: Number((recoveryTicks * 0.05).toFixed(2)),
  progressConsumed: Number((flight.progress - startProgress).toFixed(4)),
  events: events.filter((event) => event.type.startsWith('rejected-takeoff') || event.type.startsWith('recovery-')).map((event) => event.type),
}));
`;
