...
assert(live.turnaround.tasks.some((task) => task.status === 'active'), 'gate arrival has no concurrent active services');
...
assert(harness.runUntil((snapshot) => snapshot.flights.some((flight) => flight.id === arrival.id && flight.turnaround.status === 'ready'), 240), 'turnaround never reached ready state');
