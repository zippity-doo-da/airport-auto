...
assert(flight.navigation.routeClearance?.status === 'sent' && flight.navigation.readbackStatus === 'sent' && flight.navigation.routeClearance.supplements.length === 2, 'compound package skipped the Sent transport state');
assert(flight.navigation.routeFixIds.join('>') === originalRoute && flight.navigation.assignedAltitudeFt === originalAltitude && flight.navigation.assignedSpeedKts === originalSpeed, 'sent compound package partially applied');
let snapshot = digitalClearanceSnapshot(simulation.state);
const sent = snapshot.messages.find((message) => message.kind === 'compound-clearance');
assert(sent?.status === 'sent' && sent.parameters.altitudeFt === targetAltitudeFt && sent.parameters.speedKts === targetSpeedKts, 'compound package did not project as one Sent digital message');
assert(!simulation.acceptRouteReadback(flight.id), 'compound package accepted a response before delivery');
while (flight.navigation.routeClearance?.status === 'sent') simulation.update(0.1);
assert(flight.navigation.routeClearance?.status === 'pending-readback' && flight.navigation.routeClearance.deliveredAtSeconds >= flight.navigation.routeClearance.deliveryDueSeconds, 'compound package did not enter Delivered/readback state');
...
assert(delivered?.status === 'delivered' && delivered.response.deliveredAtSeconds === flight.navigation.routeClearance.deliveredAtSeconds, 'compound delivery did not project its authoritative time');
...
assert(timeoutClearance?.status === 'sent' && timeoutClearance.readbackExpiresSeconds > timeoutClearance.readbackDueSeconds, 'issued package omitted its hard response deadline');
timeoutClearance.readbackDueSeconds = timeoutClearance.readbackExpiresSeconds + 10;
timeoutClearance.deliveryDueSeconds = timeoutClearance.readbackExpiresSeconds + 10;
while (['sent', 'pending-readback'].includes(flight.navigation.routeClearance?.status)) simulation.update(0.1);
