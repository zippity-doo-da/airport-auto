...
- A local protocol-1.2 browser API and `BroadcastChannel` bridge with JSON Schemas for 86 commands, explicit station authority, structured results, causal command and scripted-controller decision IDs, replay audit data, controller policy/evaluation state, and API-2.x compatibility for playtests and controller agents.
