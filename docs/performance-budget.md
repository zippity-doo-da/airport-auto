...
- The reproducible Extreme ORD 3× fixed-step profile should keep p95 simulation-tick time below 10 ms on the project development machine. Run `npm run profile:runtime`; add `-- --airport=ATL` (or another hub code) to profile the airport under investigation. This is a diagnostic rather than a hardware-independent CI assertion.
