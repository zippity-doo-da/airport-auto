...
The current API version is `2.17.0`; snapshots use schema version `19`.
...
Approach owns terminal sequencing and may issue headings, altitudes, airspeeds, direct-to and route amendments, holds/EFCs, hold releases, approach clearances, and diversions. Tower owns landing, runway-entry, takeoff, and go-around clearances. Ground owns movement-area taxi routes, hold position/resume taxi, runway crossings, reroutes, and recovery. Ramp owns stands, ramp alleys, and pushback. Departures transfer Ramp → Ground → Tower → Approach; arrivals transfer Approach → Tower → Ground → Ramp. Supervisor can work every position and configure automation for unstaffed desks with `setStationAutomation`; selecting one operational desk automatically staffs the other positions so the airport continues safely. A command from the wrong station or against a flight owned on another frequency is rejected with an explicit reason. `contactStation` is the explicit frequency-change command; `handoffFlight` remains its backward-compatible atomic handoff equivalent until the later coordination/readback milestone introduces offered and accepted handoffs. Both change the authoritative `navigation.frequencyOwner` used by UI and API. `snapshot().controllers` reports effective automation and live workload for all four operational positions.

`amendRoute` accepts only IDs from `snapshot().airport.airspaceProgram.fixes`. The route must use fixes compatible with the flight's assigned runway procedure; an arrival must contain exactly one assigned final fix and finish there. Acceptance invalidates any prior approach clearance and builds a continuous vector from the live pose to the amended route. Unknown, duplicated, incompatible, excessively long, holding, late-final, and wrong-station amendments return structured rejection reasons.
