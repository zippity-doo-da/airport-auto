...
The current API version is `2.18.0`; snapshots use schema version `20`.
...
Snapshot schema 20 adds staged route previews, physical conflict warnings, pending/accepted/rejected readback state, and route-clearance provenance while retaining schema 19 controller-authored terminal/surface routes and diversion state.
