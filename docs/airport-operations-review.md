...
- Departures receive an explicit Ramp pushback clearance, move backward from the stand with a connected tug while engines start, transfer to Ground at the movement-area boundary, taxi to a hold-short point, transfer to Tower, line up, accelerate down a model-appropriate runway distance, rotate, and climb out.
...
- Heading, altitude, speed, direct-to, hold/EFC, approach, landing, taxi, and departure instructions pass through one command arbiter. Approach, Tower, Ground, and Ramp own distinct phases and must explicitly hand aircraft off; Supervisor can cover or automate every position.
